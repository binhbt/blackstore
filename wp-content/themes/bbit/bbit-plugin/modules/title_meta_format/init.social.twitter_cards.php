<?php
/*
* Define class bbitSocialTwitterCards
...
if (class_exists('bbitSocialTwitterCards') != true) {
    class bbitSocialTwitterCards extends bbitTitleMetaFormat
    {
        /*
        ...
        */
		...
		public function init() {
			if ( isset($this->plugin_settings['bbit_twc_use_meta']) && $this->plugin_settings['bbit_twc_use_meta']=='no' )
				return true;
		...
		public function twitter_cards_tags($ret=false) {
			...
			if ( is_home() || is_front_page() ) { //homepage
				...
				$pm = $this->the_plugin->get_theoption('bbit_title_meta_format');
			}
			if ( is_singular() ) {
				
				$pm = get_post_meta( $post->ID, 'bbit_meta', true );
				
			} else if ( is_category() || is_tag() || is_tax() ) { //taxonomy data!
				
				$__objTax = (object) array('term_id' => $post->term_id, 'taxonomy' => $post->taxonomy);

				$bbit_current_taxseo = $this->the_plugin->__tax_get_post_meta( null, $__objTax );
				if ( is_null($bbit_current_taxseo) || !is_array($bbit_current_taxseo) )
					$bbit_current_taxseo = array();

				$pm = $this->the_plugin->__tax_get_post_meta( $bbit_current_taxseo, $__objTax, 'bbit_meta' );
			}
			
			//if twitter cards are deactivated!
			if ( isset($opt['bbit_twc_use_meta']) && $opt['bbit_twc_use_meta']=='no' )
				return false;
		...
		private function default_meta($metatags, $post, $bbit_meta) {
			...
			if ( isset($metatags[ 'twitter:image' ]) && empty($metatags[ 'twitter:image' ]) ) {
				if ( isset($opt['bbit_twc_default_img']) && !empty($opt['bbit_twc_default_img']) )
					$metatags[ 'twitter:image' ] = $opt['bbit_twc_default_img'];

 				if (isset($opt['bbit_twc_image_find']) && !empty($opt['bbit_twc_image_find']) ) {
	 				// featured image
	 				if ( $opt['bbit_twc_image_find'] == 'featured' ) {
	 				...
	 				// custom field image
	 				else if ( $opt['bbit_twc_image_find'] == 'customfield' ) {
	 					$__custom_image = get_post_meta($post->ID, $opt['bbit_twc_image_customfield'], true);
	
						if ( isset($__custom_image) && !empty($__custom_image) )
							$metatags[ 'twitter:image' ] = $__custom_image;
	 				}
	 				else if ( $opt['bbit_twc_image_find'] == 'content' ) {
			...
			if ( isset($metatags[ 'twitter:image' ]) && !empty($metatags[ 'twitter:image' ]) ) {
				$__do_thumb = 'none';
				if ( isset($opt['bbit_twc_thumb_sizes']) && !empty($opt['bbit_twc_thumb_sizes']) )
					$__do_thumb = $opt['bbit_twc_thumb_sizes'];
				if ( isset($bbit_meta['bbit_twc_post_thumbsize']) && !empty($bbit_meta['bbit_twc_post_thumbsize']) )
					$__do_thumb = $bbit_meta['bbit_twc_post_thumbsize'];
		...
		private function build_thumb($image, $size) {
			...
			$iscrop = isset($opt['bbit_twc_thumb_crop']) && $opt['bbit_twc_thumb_crop']=='yes' ? true : false;
	    ...
	    * Singleton pattern
	    *
	    * @return bbitSocialTwitterCards Singleton instance
	    */
...
// Initialize the bbitSocialTwitterCards class
$bbitSocialTwitterCards = new bbitSocialTwitterCards();