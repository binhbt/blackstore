<?php
/*
* Define class bbitTitleMetaFormat
...
if (class_exists('bbitTitleMetaFormat') != true) {
    class bbitTitleMetaFormat
    {
        /*
	...
	return false;
		...
		public function head_after() {
			if ($this->do_title_rewrite()) {
				...
				if ( count($handlers) > 0 && strcasecmp($handlers[ count($handlers) - 1 ], 'bbitTitleMetaFormat::head_title_tag') == 0 ) {
					ob_end_flush();
				}
		...
		protected function format_canonical() {
        	...
        	if ( is_singular() || $__postType == 'post' ) {
				...
				$__theMeta = get_post_meta( $post->ID, 'bbit_meta', true );
        	}
				...
				$__theMeta = $this->the_plugin->__tax_get_post_meta( $bbit_current_taxseo, $__objTax, 'bbit_meta' );
        	}
				...
				$__wpquery = ( !empty($__postType) && $__postType == 'post' ? $this->post : $wp_query->get_queried_object() );
 				...
 				if ( isset($__postClean['id']) && !is_null($__postClean['id']) && $__postClean['id']>0 ) {
					...
					global $authordata;
	 				...
	 				$__tmpKeywords = get_post_meta( $__postClean['id'], 'bbit_meta', true );
	 				$__postClean['keywords'] = isset($__tmpKeywords['keywords']) ? $__tmpKeywords['keywords'] : '';
	 				$__postClean['focus_keywords'] = get_post_meta( $__postClean['id'], 'bbit_kw', true );
 				}
 			}
...
//$bbitTitleMetaFormat = new bbitTitleMetaFormat($this->cfg, ( isset($module) ? $module : array()) );
$bbitTitleMetaFormat = bbitTitleMetaFormat::getInstance();
