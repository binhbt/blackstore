...
if (class_exists('bbitServerStatusAjax') != true) {
    class bbitServerStatusAjax extends bbitServerStatus
    {
        ...
        */
        public function __construct( $the_plugin=array() )
        {
			...
			// ajax  helper
			add_action('wp_ajax_bbitServerStatusRequest', array( $this, 'ajax_request' ));
			add_action('wp_ajax_bbitServerStatusVerify', array( $this, 'verify_step' ));
		}
		...
		*/
		public function ajax_request()
		{
			...
			if( in_array( 'check_memory_limit', array_values($actions)) ){
				...
				$html = array();
            	if ( $memory < 127108864 ) {
            		$html[] = '<div class="bbit-message bbit-error">' . sprintf( __( '%s - Đề xuất thiết lập bộ nhớ ít nhất 128MB. See: <a href="%s">Increasing memory allocated to PHP</a>', $this->the_plugin->localizationName ), size_format( $memory ), 'http://codex.wordpress.org/Editing_wp-config.php#Increasing_memory_allocated_to_PHP' ) . '</div>';
            	} else {
            		$html[] = '<div class="bbit-message bbit-success">' . size_format( $memory ) . '</div>';
            	}
			...
			if( in_array( 'export_log', array_values($actions)) ){
				...
				header( 'Content-Disposition: attachment; filename="bbit-logs.html"' );
			...
			if( in_array( 'remote_get', array_values($actions)) ){
				...
				$return = array(
					...
					'html' 		=> ( $status == true ? '<div class="bbit-message bbit-success">' : '<div class="bbit-message bbit-error">' ) . $msg . '</div>' 
				);
        	}
		...
		public function verify_step() {
			...
			if ( $module == 'google_analytics' ) {
				...
				if ( $action == 'step2' ) {
					...
					$auth = $bbitGoogleAnalytics->makeoAuthLogin();
				...
				if ( $action == 'step3' ) {
					...
					$getProfile = $bbitGoogleAnalytics->get_profiles();
				...
				if ( $action == 'step4' ) {
					...
					$getGraph = $bbitGoogleAnalytics->ajax_request();
			...
			if ( $module == 'serp' ) {
				...
				if ( $action == 'step2' ) {
					...
					$getGraph = $bbitSERP->addToReporter();
			...
			if ( $module == 'pagespeed' ) {
				...
				if ( $action == 'step2' ) {
					...
					$getGraph = $bbitPagespeed->check_page( $_REQUEST['link'], $_REQUEST['postid'] );
			...
			if ( $module == 'facebook_planner' ) {
				...
				if ( $action == 'step2' ) {
					...
					$auth = $bbitFacebook_Planner->makeoAuthLogin();
