<?php
/*
* Define class bbitGoogleAuthorship
...
if (class_exists('bbitGoogleAuthorship') != true) {
	class bbitGoogleAuthorship
	{
		/*
		...
		*/
		...
		/**
	    	* Singleton pattern
	    	*
	    	* @return bbitGoogleAuthorship Singleton instance
	    	*/
		...
		*/
		public function __construct()
		{
			...
			if ( $this->the_plugin->is_admin === true ) {
				// ajax handler
				// add_action('wp_ajax_bbitGoogleAuthorship_test', array( &$this, 'test' ));
			} else {
		...
		public function set_author_link( $user_id, $force_loc=false ) {
			...
			$user_meta = get_user_meta( $user_id, 'bbit_google_authorship', true );
	    ...
	    protected function _registerMetaBoxes()
	    {
	    	if ( $this->the_plugin->capabilities_user_has_module('google_authorship') ) {
			    ...
			    foreach ($screens as $screen) {
			        ...
			        add_meta_box(
			            'bbit_google_authorship_meta_box',
...
?>
			...
			<div class="bbit-meta-box-container" id="bbit-user-authorship" style="display:none; border:0px;">
				<!-- box Tab Menu -->
				<!--<div class="bbit-tab-menu">
					<a href="#dashboard" class="open">PSP Google+ Authorship</a>
				</div>-->
			...
			$options = array(
				array(
					...
					'google_authorship_meta' => array(
						'title' 	=> __('Google+ Authorship Settings', $bbit->localizationName),
		...
		public function user_profile_save_metabox( $user_id ) 
		{
			if( 1 ) {
				if ( 1 ) {
					...
					update_user_meta( $user_id, 'bbit_google_authorship', $user_meta );
				}
			}
		}
...
// Initialize the your bbitGoogleAuthorship
//$bbitGoogleAuthorship = new bbitGoogleAuthorship();
$bbitGoogleAuthorship = bbitGoogleAuthorship::getInstance();