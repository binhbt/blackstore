...
(function init() {
	...
	$(document).ready(function() {
		ajaxurl = bbitSocialSharing_ajaxurl;

		toolbars = {
			'floating'				: $('.bbit-sshare-wrapper').filter('.box-floating'),
			'content_horizontal'	: $('.bbit-sshare-wrapper').filter('.box-panel'),
			'content_vertical'		: $('.bbit-sshare-wrapper').filter('.box-panel-vertical')
		};
...
function createToolbars() {
	var opt = $('#bbit-sshare-toolbars-options').data('options');
...
function getCount( opt, requestPerItem ) {
	...
	if ( requestPerItem ) {
			for (var key in urlList) {
				...
				jQuery.post(ajaxurl, {
					'action' 		: 'bbitSocialSharingFrontend',
					'sub_action' 	: 'getCount',
	...
	function build_position( doRecursion ) {
		...
		switch (o.type) {
			...
			case 'content_vertical' :
				...
				var $elMarkTop = $('body').find( $elContent ).find('.bbit-social-content-mark-top'),
				$elMarkBottom = $('body').find( $elContent ).find('.bbit-social-content-mark-bottom');
...
function getCount_delicious() {
	// get count info
	var $btn = $('.bbit-sshare-wrapper').find('.social-btn.delicious'), $a = $btn.find('a'), __url = $a.data('url'), __title = $a.data('title');
...
var socialButtons = {
	...
	,btnEvent: function( opt, pms ) {
		...
		currentToolbar/*$('.bbit-sshare-wrapper').filter(function () {
