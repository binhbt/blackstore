...
if (class_exists('bbitSmushit') != true) {
    class bbitSmushit
    {
        /*
        ...
        */
		...
		private static $SMUSHIT_ACTION_URL = '<a href="admin.php?action=bbit_smushit&amp;id=%s" class="bbit-smushit-action" data-itemid="%s">%s</a>';
        ...
        */
        public function __construct()
        {
			...
			$this->settings = $this->the_plugin->get_theoption( 'bbit_smushit' );
	    ...
	    */
        ...
        public function init() {
			...
			if (is_admin()) {
				...
				//add_action( 'admin_action_bbit_smushit', array( $this, 'smushit' ) );
				
				// smushit bulk action!
				//add_action( 'admin_menu', array( $this, 'add_page_smushit_bulk' ) );
				//add_action( 'admin_action_bbit_smushit_bulk', array( $this, 'goto_page_smushit_bulk' ) );
        	}
			...
			if ( $this->the_plugin->is_admin === true )
				add_action('wp_ajax_bbit_smushit', array( $this, 'ajax_request' ));
        }
	    ...
	    */
		...
		{
	    	if( !wp_script_is('bbit-media-smushit-js') ) {
	    		wp_enqueue_style( 'bbit-media-smushit-js', $this->module_folder .  'app.css', false, '1.0', 'all' );
	    	}
	    	if( !wp_script_is('bbit-media-smushit-css') ) {
				wp_enqueue_script( 'bbit-media-smushit-css', $this->module_folder . 'app.class.js', array('jquery'), '1.0', false );
	    	}
        ...
        private function meta_smushit_media_sizes( $id=null, $meta=array(), $force=false ) {
			...
			$ms = isset($meta['bbit_smushit']) ? $meta['bbit_smushit'] : '';
			if ( $force || $this->make_resmush( $ms ) ) {

				$__meta['bbit_smushit'] = $this->execute_smushit( $mediaAtts['path'], $mediaAtts['url'], $msghead );
				update_post_meta( $id, 'bbit_smushit_status', $__meta['bbit_smushit']['status'] );

				$alreadySmushit = $this->already_smushed( $ms, $__meta['bbit_smushit'], $msghead );
				if ( $alreadySmushit['status'] )
					$__meta['bbit_smushit']['msg'] = $alreadySmushit['msg'];
				
				if ( $__meta['bbit_smushit']['status']=='invalid' ) // errors occurred!
					$__meta['bbit_smushit_errors']++;
			}
			...
			foreach ( $meta['sizes'] as $key => $val ) {
				...
				$ms_size = '';
				if ( isset($val['bbit_smushit']) )
					$ms_size = $val['bbit_smushit'];
				...
				$__meta['sizes'][$key]['bbit_smushit'] = $this->execute_smushit( $mediaAtts[$key]['path'], $mediaAtts[$key]['url'], $msghead );

				$alreadySmushit = $this->already_smushed( $ms_size, $__meta['sizes'][$key]['bbit_smushit'], $msghead );
				if ( $alreadySmushit['status'] )
					$__meta['sizes'][$key]['bbit_smushit']['msg'] = $alreadySmushit['msg'];

				if ( $__meta['sizes'][$key]['bbit_smushit']['status']=='invalid' )
					$__meta['bbit_smushit_errors']++;
			}
        ...
        private function execute_smushit( $filepath='', $fileurl='', $msghead='' ) {
			...
			return array_merge( $ret, array('status' => 'reduced', 'msg' => sprintf( $msghead . __('tiết kiệm %01.2f%% (%s)', $this->the_plugin->localizationName), $getdata->percent, $bytes_reduced )) );
        }
	    ...
	    public function add_page_smushit_bulk( ) {
	    	...
	    	add_media_page( 'PSP Smushit bulk', 'PSP Smushit bulk', 'edit_others_posts', 'bbit-smushit-bulk-page', array( $this, 'smushit_bulk' ) );
	    }
	    ...
	    public function goto_page_smushit_bulk() {
			...
			wp_redirect( 
				add_query_arg( '_wpnonce', wp_create_nonce( 'bbit-smushit-bulk-nonce' ), admin_url( 'upload.php?page=bbit-smushit-bulk-page&goback=1&ids=' . $ids ) )
			);
			die();
	    }
		...
		public function smushit_bulk() {
			...
			if ( !wp_verify_nonce( $req['_wpnonce'], 'bbit-smushit-bulk-nonce' )
		...
		public function media_columns_head($columns) {
			$new_columns = $columns;
		    $new_columns['bbit_smushit'] = 'PSP Smushit';
		    return $new_columns;
		}
		...
		public function media_columns_body($column_name, $id) {
			...
			if ( $column_name=='bbit_smushit' && wp_attachment_is_image( $id ) ) {

				echo '<div class="bbit-smushit-wrapper">';
				echo 	'<span class="bbit-smushit-loading"></span>';
				...
				if ( isset($meta_new['bbit_smushit']) && !empty($meta_new['bbit_smushit']) ) {
					...
					echo '<span id="' . ('bbit-smushit-resp-'.$id) . '" class="' . $msg_cssClass . '">' . $msg . '</span><br />';
					printf( self::$SMUSHIT_ACTION_URL, $id, $id, __( 'smushit again!', $this->the_plugin->localizationName ) );
				} else {
					
					echo '<span id="' . ('bbit-smushit-resp-'.$id) . '" class="info">' . __( 'not processed!', $this->the_plugin->localizationName ) . '</span><br />';
...
?>
		...
		*/ ?>
		<div id="bbit-wrapper" class="fluid wrapper-bbit">
			<?php
			// show the top menu
			bbitAdminMenu::getInstance()->make_active('advanced_setup|smushit')->show_menu();
			?>
			...
			<div id="bbit-content">
				...
				<h1 class="bbit-section-headline">
					...
					<span class="bbit-section-info"><?php echo $this->module['smushit']['description'];?></span>
					<?php
					...
					if( $has_help === true ){
						...
						if( $help_type == 'remote' ){
							echo '<a href="#load_docs" class="bbit-show-docs" data-helptype="' . ( $help_type ) . '" data-url="' . ( $this->module['smushit']['help']['url'] ) . '">HELP</a>';
						} 
					} 
					?>
				</h1>
	                        	...
	                        	<div class="bbit-panel">
									...
									<div class="bbit-panel-content">
										<form class="bbit-form" id="1" action="#save_with_ajax">
											...
											<?php
											//$settings = $this->the_plugin->getAllSettings( 'array', 'bbit_smushit' );
...
// Initialize the bbitSmushit class
//$bbitSmushit = new bbitSmushit();
$bbitSmushit = bbitSmushit::getInstance();