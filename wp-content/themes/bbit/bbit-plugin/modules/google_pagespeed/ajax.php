<?php
/*
* Define class bbitPageSpeedInsightsAjax
...
if (class_exists('bbitDashboard') != true) {
	global $bbit;
	require_once( $bbit->cfg['paths']['plugin_dir_path'] . 'modules/dashboard/init.php' );
}
if (class_exists('bbitPageSpeedInsightsAjax') != true) {
    class bbitPageSpeedInsightsAjax extends bbitDashboard
    {
    	public $the_plugin = null;
		private $module_folder = null;
		private $file_cache_directory = '/bbit-page-speed';
        ...
        */
        public function __construct( $the_plugin=array() )
        {
			...
			// ajax  helper
			add_action('wp_ajax_bbitPageSpeedInsightsRequest', array( &$this, 'ajax_request' ));
		}
		...
		*/
		public function ajax_request()
		{
			...
			if( in_array( 'viewSpeedRaportById', array_values($actions)) ){
				...
				if( $settings['report_type'] == 'both' || $settings['report_type'] == 'desktop' ){
					$speed_scores['desktop'] = get_post_meta( $page_id, 'bbit_desktop_pagespeed', true );  
				}
				
				if( $settings['report_type'] == 'both' || $settings['report_type'] == 'mobile' ){
					$speed_scores['mobile'] = get_post_meta( $page_id, 'bbit_mobile_pagespeed', true );  
				}

				if(count($speed_scores) > 0 ){
					// write the header 
					$html[] = '<div class="bbit-pagespeed-header">';
					...
					foreach ($speed_scores as $key_device => $value_device) {
						...
						$html[] = 	'<div class="bbit-tab-item ' . ( $cc == 0 ? 'on' : '' ) . '" data-rel="' . ( $key_device ) . '">';
						$html[] = 		'<table>';
						$html[] = 			'<tr>';
						$html[] = 				'<td>';
						$html[] = 				'<div class="bbit-progress">';
						$html[] = 						'<div style="width:' . ( $score ) . '%" class="bbit-progress-bar ' . ( $size_class ) . '"></div>';
						$html[] = 						'<div class="bbit-progress-score">' . ( $score ) . '/100</div>';
						$html[] = 					'</div>';
						$html[] = 				'</td>';
						$html[] = 				'<td class="bbit-tab-title">';
					...
					$html[] = 	'<a href="#" class="bbit-button red bbit-close-page-detail"> ' . ( __('Close Page Report', $this->the_plugin->localizationName) ) . '</a>';
					
					$html[] = '</div>';
					
					$html[] = '<div class="bbit-pagespeed-page-content">';
					$cc = 0;
					foreach ($speed_scores as $key_device => $value_device) {
						$img = '';
						...
						$html[] = 	'<div id="bbit-pagespeed-page-' . ( $key_device ) . '" class="bbit-pagespeed-tab" style="' . ( $cc > 0 ? 'display:none' : '' ) . '">';
						$html[] = 		'<div class="left bbit-report-rules">';
						$html[] = 			'<div class="bbit-grid_4">';
						$html[] = 				'<div class="bbit-panel">';
						$html[] = 					'<div class="bbit-panel-header">';
						$html[] = 						'<div class="bbit-panel-title">' . ( __('Suggestions Summary', $this->the_plugin->localizationName) ) .'</div>';
						$html[] = 					'</div>';
						$html[] = 				'<div class="bbit-panel-content bbit-summary-box">';
						$html[] = 					'<div class="bbit-sub-panel-content">';
						$html[] = 						'<table class="bbit-what-to-do">';
						...
						if( count($rule_results) > 0 ){
							foreach ($rule_results as $key_rule => $value_rule) {
								...
								$html[] = 					'<tr>';
								$html[] = 						'<td class="bbit-icon-status">';
								...
								$html[] = 							'<i class="bbit-status-icon ' . ( $icon_status ) . '"></i>';
								$html[] = 						'</td>';
								$html[] = 						'<td>';
								$html[] = 							'<a href="#" class="bbit-criteria">' . ( $value_rule['localizedRuleName'] ) . '</a>';
								if( count($value_rule['urlBlocks']) > 0 ){
									
									$html[] = 					'<div class="bbit-desc-complete">';
									...
									foreach ($value_rule['urlBlocks'] as $key_blocks => $value_blocks) {
										...
										if( trim($hyperlink) != "" ){
											$html[] = '<a href="' . ( $hyperlink ) . '" class="bbit-button gray" target="_blank">' . ( __('Read Documentation', $this->the_plugin->localizationName) ) . '</a>';
										}
										$html[] = '</li>';
						...
						<div class="bbit-grid_4">
							<div class="bbit-panel">
								<div class="bbit-panel-header">
									<div class="bbit-panel-title">' . ( __('Understanding the Rule Icons', $this->the_plugin->localizationName) ) . '</div>
								</div>
								<div class="bbit-panel-content bbit-statistics-box">
									<div class="bbit-sub-panel-content">
										<table class="bbit-table">
											<tbody>
												<tr>
													<td class="bbit-icon-status">
														<i class="bbit-status-icon is_error"></i>
													</td>
												...
												<tr>
													<td class="bbit-icon-status">
														<i class="bbit-status-icon is_warning"></i>
													</td>
												...
												<tr>
													<td class="bbit-icon-status">
														<i class="bbit-status-icon is_success"></i>
													</td>
						...
						$html[] = 			'<div class="bbit-panel">';
						$html[] = 				'<div class="bbit-panel-header">';
						$html[] = 					'<div class="bbit-panel-title">';
						...
						$html[] = 		'<div class="bbit-panel">';
						$html[] = 			'<div class="bbit-panel-header">';
						$html[] = 				'<div class="bbit-panel-title">' . ( __('Page Statistics', $this->the_plugin->localizationName) ) . '</div>';
						$html[] = 			'</div>';
						$html[] = 		'<div class="bbit-panel-content bbit-statistics-box">';
						$html[] = 			'<div class="bbit-sub-panel-content">';
						$html[] = 				'<table class="bbit-table">';
						$html[] = 					'<tr>';
						...
						$html[] = 	'<div class="bbit-panel">';
						$html[] = 		'<div class="bbit-panel-header">';
						$html[] = 			'<div class="bbit-panel-title">' . ( __('Page Resources Usages', $this->the_plugin->localizationName) ) . '</div>';
						$html[] = 		'</div>';
						$html[] = 		'<div class="bbit-panel-content bbit-resources-box">';
						$html[] = 			'<div class="bbit-sub-panel-content">';
						$html[] = 				'<div id="bbit-' . ( $key_device ) . '-graph" style="height: 200px;width: 100%; "></div>';
						$html[] = '
							<script>
								data = [];
								data[0] = {
								...
								jQuery.plot(jQuery("#bbit-' . ( $key_device ) . '-graph"), data,
								{
							        series: {
							            pie: {
							                show: true
		...
		public function check_page( $url='', $page_id=0 )
		{
			...
			if( $settings['report_type'] == 'both' || $settings['report_type'] == 'desktop' ){
				...
				$response = get_post_meta( $page_id, 'bbit_desktop_pagespeed', true );
				...
				update_post_meta( $page_id, 'bbit_desktop_pagespeed', $response );
			...
			if( $settings['report_type'] == 'both' || $settings['report_type'] == 'mobile' ){
				...
				// try to get from cache
				$response = get_post_meta( $page_id, 'bbit_mobile_pagespeed', true );  
				if ( $this->cache_lifetime > 0 && ( time() > (isset($response['_create_at']) ? $response['_create_at'] : 0) + $this->cache_lifetime ) ) {
				...
				// save add create at key and save the results to the DB
				$response['_create_at'] = time();
				update_post_meta( $page_id, 'bbit_mobile_pagespeed', $response ); 
