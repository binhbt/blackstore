...
class bbitSeoSitemap
{
    ...
    */
	...
	private $file_cache_directory = '/bbit-videos';
    ...
    * Singleton pattern
    *
    * @return bbitSeoSitemap Singleton instance
    */
	...
	private function itemIsIncluded( $dbAlias='a' ) {
		...
		$sql = "
			SELECT a.post_id
			 ...
			 WHERE 1=1
			 AND a.meta_key = 'bbit_sitemap_isincluded' AND a.meta_value = 'always_include'
	...
	private function filter_item_video( $content ) {
		...
		if ( !empty($this->the_plugin->charset) )
			$doc = bbitphpQuery::newDocument( $content, $this->the_plugin->charset );
		else
		...
		foreach( $embeds2 as $tag ) {
			$tag = bbitPQ($tag); // cache the object
		...
		foreach( $objects as $tag ) {
			...
			foreach( $tag as $param ) {
				$param = bbitPQ($param); // cache the object
	...
	private function getVideosInfo( $videos = array(), $post = null, $extrainfo = array(), $recheckVideos = false ) {
		...
		$bbitVideoInfo = new bbitVideoInfo( array(
		...
		foreach ( $videos as $k => $v ) {
			...
			if ( is_array($v) && !empty($v) ) {
				foreach ( $v as $key => $val ) {
					...
					$meta_alias = 'bbit_videos_' . $current_alias;
					...
					if ( $__doRequestInfo ) {
						$ret[ "$k" ][ "$key" ] = $bbitVideoInfo->getVideoInfo( $val, $k, array(
							'post'			=> $post,
	...
	private function getVideoMetas( $post_id ) {
		...
		$sql = "
			SELECT a.*
			 FROM " . $wpdb->prefix . "postmeta AS a
			 WHERE 1=1
			 AND a.post_id = '" . $post_id . "' AND a.meta_key regexp 'bbit_videos_'
	...
	private function get_post_videos( $post=null, $recheckVideos=true ) {
		...
		if ( !empty($current_metas) ) {
			foreach ( $current_metas as $k => $v ) {
				$alias = str_replace('bbit_videos_', '', $k);
	...
	*/
	...
	private function print_sitemap( $post_type='post,page' )
	{
		...
		$general_sitemap_settings = $this->the_plugin->get_theoption('bbit_sitemap');
		...
		if( count($items) > 0 ) {
			...
			foreach ($items as $key => $value) {
				...
				$sitemap_settings = get_post_meta( $value->ID, 'bbit_sitemap', true );
				$sitemap_isincluded = get_post_meta( $value->ID, 'bbit_sitemap_isincluded', true );
