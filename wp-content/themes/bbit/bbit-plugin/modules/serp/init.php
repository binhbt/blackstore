...
if (class_exists('bbitSERP') != true) {
    class bbitSERP
    {
        ...
        */
        public function __construct()
        {
			...
			if ( $this->the_plugin->is_admin === true ) {
				add_action('wp_ajax_bbitGetFocusKW', array( &$this, '__getFocusKW' ));
				add_action('wp_ajax_bbitAddToReporter', array( &$this, 'addToReporter' ));
				add_action('wp_ajax_bbitRemoveFromReporter', array( &$this, 'removeFromReporter' ));
				add_action('wp_ajax_bbitUpdateToReporter', array( &$this, 'updateToReporter' ));
				add_action('wp_ajax_bbitGetSERPGraphData', array( &$this, 'getSERPGraphData' ));
				add_action('wp_ajax_bbitSetSearchEngine', array( &$this, 'setSearchEngine' ));
				
				add_action('wp_ajax_bbitGetEngineAccessTime', array( &$this, '__getEngineAccessTime' ));
			}
			...
			// wp_schedule_event(time(), 'daily', 'bbit_start_cron_serp_check'); //plugin activation daily|hourly
			// add_action('bbit_start_cron_serp_check', array( &$this, 'check_reporter' ));
			// wp_clear_scheduled_hook('bbit_start_cron_serp_check'); //plugin deactivation
			...
			$engine = '';
			if (isset($_SESSION['bbit_serp']['search_engine']) && !empty($_SESSION['bbit_serp']['search_engine'])
			&& $_SESSION['bbit_serp']['search_engine']!='--all--')
				$engine = $_SESSION['bbit_serp']['search_engine'];
	    ...
	    * Singleton pattern
	    *
	    * @return bbitSERP Singleton instance
	    */
	    ...
	    */
		...
		public function save_visits()
		{
			...
			if ($search_engine!='' && $postId>0) {
				...
				$reporterSql = $wpdb->prepare( "SELECT a.id as rowid, a.* FROM " . ( $wpdb->prefix ) . "bbit_serp_reporter as a WHERE 1=1 AND a.search_engine=%s AND a.post_id=%s LIMIT 1", $search_engine, $postId );
				...
				if( $row_id > 0 ) {
					...
					$wpdb->update(
						$wpdb->prefix . "bbit_serp_reporter",
						array(
...
?>
		...
		<div id="bbit-wrapper" class="fluid wrapper-bbit">
			...
			<?php
			// show the top menu
			bbitAdminMenu::getInstance()->make_active('monitoring|serp')->show_menu();
			?>
			...
			<div id="bbit-lightbox-overlay">
				<div id="bbit-lightbox-container">
					<h1 class="bbit-lightbox-headline">
						...
						<span>Your focus keywords list</span>
			...
			<div id="bbit-main-loading" style="display:block;">
				<div id="bbit-loading-overlay" style="display:block;"></div>
				<div id="bbit-loading-box" style="display:block;">
			...
			<div id="bbit-content">
				...
				<h1 class="bbit-section-headline">
					...
					<span class="bbit-section-info"><?php echo $this->module['serp']['description'];?></span>
					<?php
					...
					if( $has_help === true ){
						...
						if( $help_type == 'remote' ){
							echo '<a href="#load_docs" class="bbit-show-docs" data-helptype="' . ( $help_type ) . '" data-url="' . ( $this->module['serp']['help']['url'] ) . '">HELP</a>';
						} 
					} 
					?>
				</h1>
	                        	...
	                        	<div class="bbit-panel">
	                        		<div class="bbit-panel-header">
										<span class="bbit-panel-title">
											...
											Search Engine Results Page Reporter
										</span>
									</div>
									<div class="bbit-panel-content">
										<form class="bbit-form" action="#save_with_ajax">
											<div class="bbit-form-row" id="bbit-serp-container">
												<div class="bbit-top-filters">

													<span id="bbit-select-engine-text">Google: </span>
													...
													foreach ($__seUsed as $__k=>$__v) {
														echo '<option value="'.$__v.'" '.(isset($_SESSION['bbit_serp']['search_engine']) && $__v==$_SESSION['bbit_serp']['search_engine'] ? ' selected ' : '').'>'.$__v.'</option>';
													}
													?>
													</select>

													<?php /*<input type="button" class="bbit-button blue" id="bbit-cron-ckeck" value="Cron check now">*/ ?>
													
													<div id="bbit-filter-by-date">
														<label for="bbit-filter-by-date-from">From:</label>
														<input type="text" id="bbit-filter-by-date-from" name="bbit-filter-by-date-from" value="<?php echo $this->__initialDate['from']; ?>" />
														<label for="bbit-filter-by-date-to">to</label>
														<input type="text" id="bbit-filter-by-date-to" name="bbit-filter-by-date-to" value="<?php echo $this->__initialDate['to']; ?>" />
														<input type="button" id="bbit-toggle-ku" class="bbit-button gray bbit-select-fw" value="Customize view">
														<input type="button" class="bbit-button blue" id="bbit-filter-graph-data" value="Apply Filters">
													</div>
												</div>
												
												<!-- keywords, url filter -->
												<div class="bbit-panel bbit-serp-filter-keyurl" id="bbit-serp-filter-keyurl">
					                        		<div class="bbit-panel-header">
														<span class="bbit-panel-title">
															Filter Focus Keywords and URLs
														</span>
													</div>
													<div class="bbit-panel-content bbit-serp-filter-keyurl-content">
														...
														<table class="bbit-table" style="border:  none;border-bottom:  1px solid #dadada;width:100%;  border-spacing:0;border-collapse:collapse;">
															<thead>
																<tr>
																	<th align="left" width="10"><input type="checkbox" id="bbit-item-check-all-key"></th>
																	<th align="left">Focus Keywords</th>
																</tr>
															</thead>
															...
															if (count($__keys)>0) {
																...
																foreach ($__keys as $k=>$v) {
																	...
																	$__theHtml[] = 			'<input type="checkbox" class="bbit-item-checkbox-key" name="bbit-item-checkbox-key-'.$v['id'].'" value="'.$v['info'].'"' . (isset($_SESSION['bbit_serp']['filter_keywords']) && isset($_SESSION['bbit_serp']['filter_keywords'][$v['info']]) ? ' checked="checked" ' : '') . '>';
																	$__theHtml[] = 		'</td>';
																	$__theHtml[] = 		'<td align="left">';
																	$__theHtml[] = 			$v['info'];
																	$__theHtml[] = 		'</td>';
														...
														<div style="float:left; width:50%; height: 200px; overflow: auto;">
														 <table class="bbit-table" style="border: none;border-left: 1px solid #ededed;border-bottom: 1px solid #dadada;width:100%;border-spacing:0;border-collapse:collapse;">
															<thead>
																<tr>
																	<th align="left" width="10"><input type="checkbox" id="bbit-item-check-all-url"></th>
															...
															if (count($__keys)>0) {
																...
																foreach ($__keys as $k=>$v) {
																	$__theHtml[] = '<tr>';
																	$__theHtml[] = 		'<td align="left">';
																	$__theHtml[] = 			'<input type="checkbox" class="bbit-item-checkbox-url" name="bbit-item-checkbox-url-'.$v['id'].'" value="'.$v['info'].'"' . (isset($_SESSION['bbit_serp']['filter_urls']) && isset($_SESSION['bbit_serp']['filter_urls'][$v['info']]) ? ' checked="checked" ' : '') . '>';
																	$__theHtml[] = 		'</td>';
																	$__theHtml[] = 		'<td align="left">';
																	$__theHtml[] = 			$v['info'];
																	$__theHtml[] = 		'</td>';
												...
												<div class="bbit-serp-graph" id="bbit-serp-graph"></div>
												
												<div class="bbit-panel bbit-serp-add-keyword" id="bbit-serp-add-keyword">
					                        		<div class="bbit-panel-header">
														<span class="bbit-panel-title">
															<img src="<?php echo $this->module_folder;?>assets/new-kw.png">
															Add Keyword <i>(You can keep an eye on your competitors too)</i>
															<span id="search-engine-current-loc">Google location currently used: ', $this->the_plugin->localizationName); echo '<strong>'.$this->search_engine."</strong>"; ?></span>
														</span>
													</div>
													<div class="bbit-panel-content">
														<div id="bbit-add-keyword-block">
															<div style="float: left;width: 450px;">
																<label for="bbit-new-keyword">Keyword:</label>
																<input type="text" id="bbit-new-keyword" name="bbit-new-keyword" class="bbit-new-keyword" value="" />
																<div style="clear:left;"></div>
																<label for="bbit-new-keyword-link">URL:</label>
																<input type="text" id="bbit-new-keyword-link" name="bbit-new-keyword-link" class="bbit-new-keyword-link" value="" />
															</div>
															<div class="bbit-or-block">
																<span class="line"></span>
																<h2>OR</h2>	
															</div>
															<div style="float: left;width: 300px; ">
																<label>Select from your:</label>
																<div style="clear:left;"></div>
																<input type="button" id="bbit-select-fw" class="bbit-button gray bbit-select-fw" value="Focus keywords">
															</div>	
															<div style="clear:left;"></div>
															<input type="button" class="bbit-button blue" id="bbit-submit-to-reporter" value="Add to Reporter">
														</div>
														<div style="clear:left;"></div>
													</div>
												</div>
												...
												<form class="bbit-form" action="#save_with_ajax">
													...
													bbitAjaxListTable::getInstance( $this->the_plugin )
														->setup(array(
															'id' 				=> 'bbitSERPKeywords',
															'custom_table'		=> "bbit_serp_reporter",
															'show_header' 		=> true,
															'items_per_page' 	=> '10',
															'post_statuses' 	=> 'all',
															'columns'			=> array(
...
<?php
		...
		* ------------------------
		*
		* this will create request to bbit_serp_reporter table
		*/
		public function getSERPGraphData()
		{
			...
			//Query
			$get_ranks_sql = $wpdb->prepare( "SELECT a.*, b.* FROM " . ( $wpdb->prefix ) . "bbit_serp_reporter as a LEFT JOIN " . ( $wpdb->prefix ) . "bbit_serp_reporter2rank as b ON a.id=report_id WHERE 1=1 " . ($__dose ? " AND a.search_engine='".$request['engine']."' " : " ") . $q_keyurl_clause . " AND b.report_day BETWEEN %s and %s order by b.report_day DESC;", $request['from_date'], $request['to_date'] );
		...
		public function getDefaultClause( $request ) {
			...
			//$q_keyurl_clause = " and a.focus_keyword in (select c.focus_keyword from " . ( $wpdb->prefix ) . "bbit_serp_reporter as c where 1=1 group by c.focus_keyword order by c.created desc limit 5) ";
			/*$__latestKeyQuery = "select c.focus_keyword from " . ( $wpdb->prefix ) . "bbit_serp_reporter as c where 1=1 " . ($__dose ? " AND c.search_engine='".$request['engine']."' " : " ") . " group by c.focus_keyword order by c.created desc limit 5;";
			...
			$__latestKeyQuery = $wpdb->prepare( "SELECT a.focus_keyword, a.url FROM " . ( $wpdb->prefix ) . "bbit_serp_reporter as a LEFT JOIN " . ( $wpdb->prefix ) . "bbit_serp_reporter2rank as b ON a.id=report_id WHERE 1=1 " . ($__dose ? " AND a.search_engine='".$request['engine']."' " : " ") . " AND b.report_day BETWEEN %s and %s GROUP BY a.focus_keyword, a.url order by b.report_day DESC LIMIT 5;", $request['from_date'], $request['to_date'] );
			...
			if (is_array($__latestKey) && count($__latestKey)>0) {
				foreach ($__latestKey as $k=>$v) {
					...
					$_SESSION['bbit_serp']['filter_keywords']["{$v['focus_keyword']}"] = true;
					$_SESSION['bbit_serp']['filter_urls']["{$v['url']}"] = true;
				}
				$__tmp = array();
		...
		public function getInitialData() {
			...
			$sql = "
				SELECT COUNT(b.id) AS nb, b.report_day FROM " . ( $wpdb->prefix ) . "bbit_serp_reporter AS a LEFT JOIN
				 " . ( $wpdb->prefix ) . "bbit_serp_reporter2rank AS b ON a.id=report_id WHERE 1=1
				 AND b.position>0
		...
		*/
		...
		{
			$request = array(
				'action'	=> isset($_REQUEST['action']) ? trim($_REQUEST['action']) : 'bbitAddToReporter',
			...
			$serp = bbitSERPCheck::getInstance(); 
			...
			if ( $googleScoreInfo===false || ( isset($googleScoreInfo['status']) 
				...
				if ( $request['action'] == 'bbitUpdateToReporter' && $request['itemid'] > 0 ) { //update error message only for update!
				...
				$_SESSION['bbit_engine_access_status'] = 'invalid';
			...
			if ( $request['action'] == 'bbitUpdateToReporter' ) {
				//keep page number & items number per page
				$_SESSION['bbitListTable']['keepvar'] = array('paged'=>true,'posts_per_page'=>true);
			}
			...
			$_SESSION['bbit_engine_access_status'] = 'valid';
		...
		*/
		public function get_serp_scores( $kw='', $link='', $se='default' )
		{
			...
			$serpScoresSQL = $wpdb->prepare( "SELECT a.*, b.* FROM " . ( $wpdb->prefix ) . "bbit_serp_reporter as a LEFT JOIN " . ( $wpdb->prefix ) . "bbit_serp_reporter2rank as b ON a.id=report_id WHERE 1=1 AND a.focus_keyword=%s AND a.url=%s AND a.search_engine=%s;", $kw, $link, $se );
		...
		*/
		public function updateToReporter()
		{
			...
			if( $request['itemid'] > 0 ){
				$row = $wpdb->get_row( "SELECT * FROM " . ( $wpdb->prefix ) . "bbit_serp_reporter WHERE id = '" . ( $request['itemid'] ) . "'", ARRAY_A );
		...
		*/
		public function removeFromReporter()
		{
			...
			if( $request['itemid'] > 0 ){
				...
				$wpdb->delete( 
					$wpdb->prefix . "bbit_serp_reporter2rank", 
		...
		*/
		public function addToReportDB( $scoreArray=array(), $search_engine='default', $itemid=0 )
		{
			...
			// check if you already have this info into DB 
			$checkSQL = $wpdb->prepare( "SELECT a.id as rowid, a.*, b.* FROM " . ( $wpdb->prefix ) . "bbit_serp_reporter as a LEFT JOIN " . ( $wpdb->prefix ) . "bbit_serp_reporter2rank as b ON a.id=b.report_id WHERE 1=1 AND a.focus_keyword=%s AND a.url=%s AND a.search_engine=%s order by b.report_day DESC LIMIT 1", $scoreArray['keyword'], $scoreArray['url'], $search_engine );
			...
			// update rank for the same report row - same day!
				...
				$row2 = $wpdb->get_row( "SELECT * FROM " . ( $wpdb->prefix ) . "bbit_serp_reporter2rank WHERE report_id = '" . ( $row_id ) . "' and report_day='" . ( $today ) . "'", ARRAY_A );
			...
			if ( $request['wait_time'] > 0 )
				$_SESSION['bbit_engine_access_time'] = $request['wait_time'];
		...
		public function getCustomRanks($report_id) {
			...
			// get best rank
			$best_rank_data = $this->the_plugin->db->get_row( "SELECT position FROM " . ( $this->the_plugin->db->prefix ) . "bbit_serp_reporter2rank where 1=1 and report_id='" . ( $report_id ) . "' order by position asc limit 1;", ARRAY_A );
			/* and position>0
			...
			// get worst
			$worst_rank_data = $this->the_plugin->db->get_row( "SELECT position FROM " . ( $this->the_plugin->db->prefix ) . "bbit_serp_reporter2rank where 1=1 and report_id='" . ( $report_id ) . "' order by position desc limit 1;", ARRAY_A );
			/*$worst_rank_data = $this->the_plugin->db->get_row( "SELECT position FROM " . ( $this->the_plugin->db->prefix ) . "bbit_serp_reporter2rank where 1=1 and report_id='" . ( $report_id ) . "' and position=0 limit 1;", ARRAY_A );
		...
		{
			$last_msg = '';
			if ( isset($_SESSION['bbit_engine_access_status']) ) {
				
				if ( $_SESSION['bbit_engine_access_status']=='valid' ) {
					$last_msg = '<div class="bbit-message bbit-success">';
					$last_msg .= __('<span class="engine-access-msg-success">' . 'Response received from Google API.' . '</span>', $this->the_plugin->localizationName);
					
				} else {
					$last_msg = '<div class="bbit-message bbit-error">';
			...
			$currentReqInfo = get_option('bbit_serp_nbrequests');
			$currentNbReq = (int) $currentReqInfo['nbreq'];
			$currentData = $currentReqInfo['data'];

			die( json_encode(array(
				'status' 	=> 'valid',
				'data' 		=> isset($_SESSION['bbit_engine_access_time']) && $_SESSION['bbit_engine_access_time']>0 ? $_SESSION['bbit_engine_access_time'] : 0,
				'last_msg'	=> $last_msg,
		...
		*/
		public function __getFocusKW()
		{
			...
			$html[] = '<table class="bbit-table" style="width: 100%;border: 1px solid #dadada;background: #fff;margin: -10px 0px 0px 0px;" cellspacing="0" cellpadding="0">'; 
			$html[] = 	'<thead>'; 
			$html[] = 		'<tr>'; 
			...
			$results = $wpdb->get_results( "SELECT  * FROM " . ( $wpdb->prefix ) . "postmeta WHERE `meta_key` = 'bbit_kw' and meta_value != '' ", ARRAY_A);
			$html[] = '<tbody>'; 
			if( count($results) > 0 ){
				foreach ($results as $key => $value){
					...
					$html[] = 	'<td><input type="button" data-itemid="' . ( $value['post_id'] ) . '" data-permalink="' . ( $permalink ) . '" data-keyword="' . ( $value['meta_value'] ) . '" value="' . __('Add to Reporter', $this->the_plugin->localizationName) . '" class="bbit-button blue bbit-this-select-fw"></td>';
					$html[] = '</tr>';
				}
			}else{
		...
		* --------------------
		*
		* this will check search engine rank for all rows in bbit_serp_reporter
		*/
		public function check_reporter() {
			...
			$sql = "SELECT a.id, a.focus_keyword, a.search_engine, COUNT(a.id) AS nb FROM " . ( $wpdb->prefix ) . "bbit_serp_reporter as a WHERE 1=1 and a.publish='Y' GROUP BY a.focus_keyword, a.search_engine HAVING nb>1 ORDER BY a.id ASC;";
			...
			$sql2 = "SELECT a.url, a.id, a.focus_keyword, a.search_engine, COUNT(a.id) AS nb FROM " . ( $wpdb->prefix ) . "bbit_serp_reporter as a WHERE 1=1 and a.publish='Y' GROUP BY a.focus_keyword, a.search_engine HAVING nb<=1 ORDER BY a.id ASC;";
			...
			foreach ($__tasks as $__k => $__v) {
				foreach ($__v as $key => $value) {
					if ($__k==0) { //use cache!
						...
						$sql_url = $wpdb->prepare( "SELECT a.id, a.focus_keyword, a.url, a.search_engine FROM " . ( $wpdb->prefix ) . "bbit_serp_reporter as a WHERE 1=1 and a.publish='Y' and a.focus_keyword=%s and a.search_engine=%s ORDER BY a.id ASC;", $value['focus_keyword'], $value['search_engine'] );
		...
		*/
		public function cron_reporter_email() {
			...
			// select from DB
			$myQuery = "SELECT a.* FROM " . ( $wpdb->prefix . "bbit_serp_reporter" ) . " as a WHERE 1=1 ";
		    ...
		    foreach ($query_res as $key => $myrow){
		    	//if( $opt["custom_table"] == 'bbit_serp_reporter' ) {
		...
		private function getSearchEngineUsed() {
			...
			$serpScoresSQL = "SELECT a.search_engine FROM " . ( $wpdb->prefix ) . "bbit_serp_reporter as a WHERE 1=1 GROUP BY a.search_engine;";
		...
		public function setSearchEngine() {
			...
			if ($request['search_engine']!='') {
				$_SESSION['bbit_serp']['search_engine'] = $request['search_engine'];
			}
		...
		private function getKeywordsList() {
			...
			$serpScoresSQL = "SELECT a.id, a.focus_keyword as info FROM " . ( $wpdb->prefix ) . "bbit_serp_reporter as a WHERE 1=1 ";

			if (isset($_SESSION['bbit_serp']['search_engine']) && !empty($_SESSION['bbit_serp']['search_engine'])
			&& $_SESSION['bbit_serp']['search_engine']!='--all--') {
				$serpScoresSQL = str_replace("1=1 ", " 1=1 and a.search_engine='".$_SESSION['bbit_serp']['search_engine']."' ", $serpScoresSQL);
			}
		...
		private function getUrlsList() {
			...
			$serpScoresSQL = "SELECT a.id, a.url as info FROM " . ( $wpdb->prefix ) . "bbit_serp_reporter as a WHERE 1=1 ";
...
function bbitSERP_cronReporter_event() {
	// Initialize the bbitSERP class
	$bbitSERP = new bbitSERP();
	$bbitSERP->check_reporter();
}

// Initialize the bbitSERP class
//$bbitSERP = new bbitSERP();
$bbitSERP = bbitSERP::getInstance();