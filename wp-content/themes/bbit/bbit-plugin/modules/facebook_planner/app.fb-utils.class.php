 ...
 * http://bbit.vn
 *
 * @package			bbit_fbPlannerUtils
 * @author			Bbit
 */

// Plugin facebook SDK load
global $bbit;
require_once ( $bbit->cfg['paths']['scripts_dir_path'] . '/facebook/facebook.php' );

class bbit_fbPlannerUtils
{
    ...
    public static function getInstance()
    {
        if (!isset(self::$instance)) {
            self::$instance = new bbit_fbPlannerUtils;
        }
	...
	public function __construct(){
		...
		// create utils
		self::$utils = array(
			'token'		=> get_option('bbit_fb_planner_token'),
	...
	public function getPostByID($id){
		if((int)$id > 0){
			return array(
				'name' 			=> get_post_meta($id, 'bbit_wplannerfb_title', true),
				'link' 			=> get_post_meta($id, 'bbit_wplannerfb_permalink', true),
				'description' 	=> get_post_meta($id, 'bbit_wplannerfb_description', true),
				'caption' 		=> get_post_meta($id, 'bbit_wplannerfb_caption', true),
				'message' 		=> get_post_meta($id, 'bbit_wplannerfb_message', true),
				'picture'	 	=> get_post_meta($id, 'bbit_wplannerfb_image', true),
				'use_picture'	=> get_post_meta($id, 'bbit_wplannerfb_useimage', true)
			);
		}
		return array();
	}
