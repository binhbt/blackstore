...
if (class_exists('bbitLocalSEO') != true) {
    class bbitLocalSEO
    {
        /*
        ...
        */
		...
		static protected $slug = 'bbitlocation';
        ...
        */
		...
		public function init_postType() 
		{
		    ...
		    $labels = array(
		        ...
		        'singular_name' 		=> __('bbit location', $this->the_plugin->localizationName),
		    ...
		    register_post_type('bbit_locations', $args);
			...
			if( isset($_GET['post_type']) && $_GET['post_type'] == 'bbit_locations') 
	    ...
	    */
	    ...
	    {
	    	$screens = array(
	    		'bbit_locations' => __( 'PSP Locations Details', $this->the_plugin->localizationName )
	    	);
		    foreach ($screens as $key => $screen) {
		    	$screen = str_replace("_", " ", $screen);
				$screen = ucfirst($screen);
		        add_meta_box(
		            'bbit_locations_meta_box',
		            $screen,
		...
		public function custom_metabox() {
			...
			$locations_meta = get_post_meta( $post_id, 'bbit_locations_meta', true );
		...
		?>
			...
			<div class="bbit-meta-box-container" style="display:none;">
				...
				<div class="bbit-tab-menu">
					<a href="#business_information" class="open">Business Information</a>
					<a href="#business_contact">Business Contact and Google Map</a>
					<a href="#opening_hours">Opening Hours</a>
					<a href="#other_details">Other details</a>
				</div>
				
				<div class="bbit-tab-container">

					<div id="bbit-tab-div-id-business_information" style="display:block;">
					...
					<div id="bbit-tab-div-id-business_contact" style="display:none;">
						<div class="bbit-dashboard-box span_3_of_3">
							<!-- Creating the option fields -->
							<div class="bbit-form">
					...
					<div id="bbit-tab-div-id-opening_hours" style="display:none;">
						<div class="bbit-dashboard-box span_3_of_3">
							<!-- Creating the option fields -->
							<div class="bbit-form">
					...
					<div id="bbit-tab-div-id-other_details" style="display:none;">
						<div class="bbit-dashboard-box span_3_of_3">
							<!-- Creating the option fields -->
							<div class="bbit-form">
					...
					'location' => array(
						...
						'elements'	=> array(
							...
							,'openings_hours_msginfo' => array(
								'type' 		=> 'html',
								'html' 		=> '<div class="bbit-form-row">You must use 24h format ( hours from 0-23, minutes from 0-59 ).</div>'
							)
		...
		public function opening_hours_tpl( $openings=array() ) {
			...
			echo '<input type="hidden" name="bbit-opening-nr" id="bbit-opening-nr" value="' . ( count($openings) ) . '" />';
			?>
							<a class="bbit-button blue small" id="bbit-add-new-opening" href="#">
								<img alt="" src="<?php echo $this->the_plugin->cfg['paths']['plugin_dir_url'] . 'modules/local_seo/assets/plus.png';?>">
								Add new Opening
							</a>
			<?php
			echo '<div class="bbit-form-row" id="bbit-panel-content-dom">';
			if(is_array($openings) && count($openings) > 0) {
				foreach ($openings as $key => $value){
					$__theKey = '';
					//<div class='bbit-oh-time-slider' style='width:100px;'></div>
					echo 			"<div class='bbit-form-row'>
										<div class='bbit-form-col-3-8'>
					" . $this->opening_hours_day( "oh[".$key."][day]", $value['day'] ) . "
										</div>
										<div class='bbit-form-col-1-8'> 
										...
										<div class='bbit-form-col-1-8' style='position: relative;'> 
			...
			?>
									<div class="bbit-message bbit-info" id="bbit-opening-no-items">You currently have not entered any opening. You can do that by click on the <i>"Add new opening"</i> .</div>
			<?php 
			}
			echo '</div>';
			
			echo "
								<div id='bbit-locations-opening-tpl' style='display:none;'>
		...
		?>
				<label for='<?php echo $field_name; ?>' style=''>Day</label>
				<select class='opening-day' name='<?php echo $field_name; ?>' style='height:29px;'>
					<?php
					foreach ($bbit_days_list as $kk => $vv){
		...
		<div class="bbit-form-row">
			<label>Google Map Preview:', $this->the_plugin->localizationName); ?>	</label>
			...
			<div class="bbit-form-item large">
				<div id="bbit-map-canvas"></div>
				<a name="bbit-geocode-verify" href='#bbit-geocode-verify' class='bbit-geocode-verify'>Verify map</a>
				<div class="bbit-geocode-status" style="display: none;"></div>
			</div>
		</div>
		<?php
			$output = ob_get_contents();
		...
		public function locations_posts_columns($column_name, $id) {
			...
			// retrieve the existing value(s) for this meta field. This returns an array
			$locations_meta = get_post_meta( $id, 'bbit_locations_meta', true );
		...
		public function meta_box_save_postdata( $post_id ) 
		{
			...
			if( isset($post) ) {
				...
				if($post->post_type == 'bbit_locations') {
					...
					$nrOfOpenings = (int) $_POST['bbit-opening-nr'];
					if($nrOfOpenings > 0) {
						$openings = array();
					...
					update_post_meta( $post_id, 'bbit_locations_meta', $locations_meta );
				}
			}
		}
	    ...
	    * Singleton pattern
	    *
	    * @return bbitLocalSEO Singleton instance
	    */
...
// Initialize the bbitLocalSEO class
//$bbitLocalSEO = new bbitLocalSEO();
$bbitLocalSEO = bbitLocalSEO::getInstance();
