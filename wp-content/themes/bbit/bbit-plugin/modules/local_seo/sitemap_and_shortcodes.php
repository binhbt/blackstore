...
if (class_exists('bbitLocalSEOSitemapShortcodes') != true) {
    class bbitLocalSEOSitemapShortcodes extends bbitLocalSEO
    {
        /*
		...
		*/
        ...
        private function build_opening_hours( $value=array() ) {
			...
			if ( count($value['oh']) > 0 ) {
				...
				foreach ( $value['oh'] as $k => $val ) {
					...
					//$ret[] = '<link itemprop="hasOpeningHoursDayOfWeek" href="http://purl.org/goodrelations/v1#'.$bbit_days_list[ "{$val['day']}" ].'" />';
			     	//$ret[] = '<meta itemprop="opens" content="'.$__from2.'">'.$__from.' - <meta itemprop="closes" content="'.$__to2.'">'.$__to;
			     	
			     	// schema.org version!
			     	$ret[] = '<link itemprop="dayOfWeek" href="http://purl.org/goodrelations/v1#' . ( isset($bbit_days_list[ "{$val['day']}" ]) ? $bbit_days_list[ "{$val['day']}" ] : '' ) . '" />' . ( isset($bbit_days_list[ "{$val['day']}" ]) ? $bbit_days_list[ "{$val['day']}" ] : '' );
			     	$ret[] = '<meta itemprop="opens" content="'.$__from2.'">'.$__from.' - <meta itemprop="closes" content="'.$__to2.'">'.$__to;
				}
        ...
        private function build_geomap( $value=array(), $atts=array() ) {
        	...
        	if ( $type == 'static' ) { // static map
				...
				if( !wp_style_is('bbit_gmap_css') ) {
					wp_enqueue_style( 'bbit_gmap_css' , $this->module_folder . '/app.frontend.css' );
				}
				...
				if( !wp_script_is('bbit_googlemap') ) {
					wp_enqueue_script( 'bbit_googlemap' , $this->get_geo_uri_js() );
				}
				if( !wp_script_is('bbit_custom_gmap') ) {
					wp_enqueue_script( 'bbit_custom_gmap' , $this->module_folder . '/app.frontend.js', array( 
						'jquery',
						'bbit_googlemap'
					) );
				}
				if( wp_script_is('jquery') && wp_script_is('bbit_googlemap') && wp_script_is('bbit_custom_gmap') ) {
				?>
				<script type="text/javascript">
				(function () {
					var t;
					var startWhenVisible = function () {
						if ( typeof bbitGoogleMap === 'undefined' ) {
							...
							jQuery('.bbit-map-canvas').makeGmap();
							//});
				...
				?>
				<div class="bbit-map-canvas" style="display: none; width: <?php echo $atts['width']; ?>px; height: <?php echo $atts['height']; ?>px;"></div>
				<div class="bbit-map-info" style="display:none;">
        ...
        public function init_shortcodes() {
			...
			add_shortcode( 'bbit_business', array( $this, 'sh_business') );
			add_shortcode( 'bbit_address', array( $this, 'sh_address') );
			add_shortcode( 'bbit_contact', array( $this, 'sh_contact') );
			add_shortcode( 'bbit_opening_hours', array( $this, 'sh_opening_hours') );
			add_shortcode( 'bbit_payment', array( $this, 'sh_payment') );
			add_shortcode( 'bbit_gmap', array( $this, 'sh_gmap') );
			add_shortcode( 'bbit_full', array( $this, 'sh_full') );
        }
        ...
        public function shortcode_header( $shortcode = null, $execute = true, $id = null ) {
        	...
        	$qId = !is_null($id) && (int) $id > 0 ? " data-itemid='bbit-$shortcode-$id' " : '';
        	
			$ret[] = '
				<!--begin bbit local seo shortcode ' . ($qId != '' ? '-' . $qId : '') . '-->
				<div itemscope itemtype="http://schema.org/LocalBusiness">
			';
			$ret = implode('', $ret);
        	return $ret;
        ...
        public function sh_business( $atts, $content = null ) {
			...
			if( count($items) > 0 ) {
				foreach ($items as $key => $value) {
					...
					$ret[] = '<div class="bbit-loc-business">';
					...
					if ( $show_img_logo && isset($value['logo_image']) && !empty($value['logo_image']) )
						$ret[] = 	'<span itemscope itemtype="http://schema.org/ImageObject" itemprop="logo"><img src="'.$value['logo_image'].'" itemprop="image" class="bbit-company-logo" /></span>';
		...
		// [bbit_gmap id=all width=320 height=240 zoom=12 maptype="roadmap"]
...
// Initialize the bbitLocalSEOSitemapShortcodes class
$bbitLocalSEOSitemapShortcodes = new bbitLocalSEOSitemapShortcodes();