...
if (class_exists('bbitLinkBuilder') != true) {
    class bbitLinkBuilder
    {
        /*
        ...
        */
        public function __construct()
        {
			...
			if ( $this->the_plugin->is_admin === true ) {
				...
				// ajax handler
				add_action('wp_ajax_bbitGetUpdateDataBuilder', array( &$this, 'ajax_request' ));
				add_action('wp_ajax_bbitAddToBuilder', array( &$this, 'addToBuilder' ));
				add_action('wp_ajax_bbitRemoveFromBuilder', array( &$this, 'removeFromBuilder' ));
				add_action('wp_ajax_bbitUpdateToBuilder', array( &$this, 'updateToBuilder' ));
				add_action('wp_ajax_bbitGetHitsByPhrase', array( &$this, 'getHitsByPhrase' ));
				
				//delete bulk rows!
				add_action('wp_ajax_bbitLinkBuilder_do_bulk_delete_rows', array( &$this, 'delete_rows' ));
			}
	    ...
	    * Singleton pattern
	    *
	    * @return bbitLinkBuilder Singleton instance
	    */
	    ...
	    */
		...
		private function getLinks() {
			...
			$result_query = "SELECT a.url, a.phrase, a.title, a.rel, a.target, a.max_replacements FROM " . $wpdb->prefix . "bbit_link_builder as a WHERE 1=1 and a.publish='Y' order by a.id asc;";
		...
		private function createTable() {
			...
			$table_name = $wpdb->prefix . "bbit_link_builder";
		...
		*/
		public function updateToBuilder()
		{
			...
			if( $request['itemid'] > 0 ) {
				$row = $wpdb->get_row( "SELECT * FROM " . ( $wpdb->prefix ) . "bbit_link_builder WHERE id = '" . ( $request['itemid'] ) . "'", ARRAY_A );
...
?>
		...
		<div id="bbit-wrapper" class="fluid wrapper-bbit">
			...
			<?php
			// show the top menu
			bbitAdminMenu::getInstance()->make_active('off_page_optimization|Link_Builder')->show_menu();
			?>
		
			<div id="bbit-lightbox-overlay">
				<div id="bbit-lightbox-container">
					<h1 class="bbit-lightbox-headline">
						<span id="link-title-details">Chi tiết:</span>
						<span id="link-title-add">Thêm liên kết mới:</span>
						<span id="link-title-upd">Cập nhật liên kết:</span>
						<a href="#" class="bbit-close-btn" title="Close Lightbox"></a>
					</h1>

					<div class="bbit-seo-status-container">
						<div id="bbit-lightbox-seo-report-response-details">
								<table width="100%">
									<tr>
									...
									<tr>
										<td><label>Số liên kết tối đa:</label></td>
						...
						<div id="bbit-lightbox-seo-report-response">
							<form class="bbit-add-link-form">
								...
								<table width="100%">
									<tr>
										<td width="80"><label>Text:</label></td>
										<td><input type="text" id="new_text" name="new_text" value="" class="bbit-add-link-field" /></td>
									</tr>
									<tr>
									...
									<tr>
										<td></td>
										<td>
											<input type="button" class="bbit-button blue" value="Kiểm tra tồn tại" id="bbit-builder-verify-hits"><span style="margin-left:10px;" id="bbit-builder-text-hits"><span style="font-weight:bold;"></span> posts|pages in which the text was found!</span>
										</td>
									</tr>
									<tr>
										<td><label>Title:</label></td>
										<td><input type="text" id="new_title" name="new_title" value="" class="bbit-add-link-field" /></td>
									</tr>
									<tr>
										<td><label>Rel:</label></td>
										<td>
									...
									<tr>
										<td></td>
										<td>
											<input type="button" class="bbit-button green" value="Thêm liên kết mới" id="bbit-submit-to-builder">
										</td>
									</tr>
								</table>
						...
						<div id="bbit-lightbox-seo-report-response2">
							<form class="bbit-update-link-form">
								...
								<table width="100%">
									<tr>
										<td width="80"><label>Text:</label></td>
										<td><input type="text" id="new_text2" name="new_text2" value="" class="bbit-add-link-field" readonly disabled="disabled" /></td>
									</tr>
									<tr>
										<td><label>URL:</label></td>
									...
									<tr>
										<td><label>Title:</label></td>
										<td><input type="text" id="new_title2" name="new_title2" value="" class="bbit-add-link-field" /></td>
									</tr>
									<tr>
										<td><label>Rel:</label></td>
										<td>
									...
									<tr>
										<td></td>
										<td>
											<input type="button" class="bbit-button green" value="Cập nhật link" id="bbit-submit-to-builder2">
										</td>
									</tr>
								</table>
			...
			<div id="bbit-content">
				...
				<h1 class="bbit-section-headline">
					...
					<span class="bbit-section-info"><?php echo $this->module['Link_Builder']['description'];?></span>
					<?php
					...
					if( $has_help === true ){
						...
						if( $help_type == 'remote' ){
							echo '<a href="#load_docs" class="bbit-show-docs" data-helptype="' . ( $help_type ) . '" data-url="' . ( $this->module['Link_Builder']['help']['url'] ) . '">HELP</a>';
						} 
					} 
					?>
				</h1>
	                        	...
	                        	<div class="bbit-panel">
	                        		<div class="bbit-panel-header">
										<span class="bbit-panel-title">
											SEO link builder (internal/external)
										</span>
									</div>
									<div class="bbit-panel-content">
										<form class="bbit-form" id="1" action="#save_with_ajax">
											...
											bbitAjaxListTable::getInstance( $this->the_plugin )
												->setup(array(
													'id' 				=> 'bbitLinkBuilder',
													'custom_table'		=> "bbit_link_builder",
													...
													'columns'			=> array(
														...
														'phrase'		=> array(
															'th'	=> __('Cụm từ', $this->the_plugin->localizationName),
															'td'	=> '%builder_phrase%',
															'align' => 'center',
															'width' => '150'
														),
														...
														'url_attributes'	=> array(
															'th'	=> __('Thuộc tính', $this->the_plugin->localizationName),
															'td'	=> '%url_attributes%',
															'align' => 'center',
															'width' => '100'
														),
														...
														'created'		=> array(
															'th'	=> __('Ngày tạo', $this->the_plugin->localizationName),
															'td'	=> '%created%',
															'width' => '115'
														),
																...
																'publish_btn' => array(
																	...
																	//'td'	=> '%button_html5data%',
																	'option' => array(
																		'value' => __('Đã chạy', $this->the_plugin->localizationName),
																		'value_change' => __('Chưa chạy', $this->the_plugin->localizationName),
																...
																'update_btn' => array(
																	'th'	=> __('Cập nhật', $this->the_plugin->localizationName),
																	'td'	=> '%button%',
																	'option' => array(
																		'value' => __('Cập nhật', $this->the_plugin->localizationName),
																...
																'delete_btn' => array(
																	'th'	=> __('Xóa', $this->the_plugin->localizationName),
																	'td'	=> '%button%',
																	'option' => array(
																		'value' => __('Xóa', $this->the_plugin->localizationName),
													...
													'mass_actions' 	=> array(
														'add_new_link' => array(
															'value' => __('Thêm liên kết mới', $this->the_plugin->localizationName),
															'action' => 'do_add_new_link',
															'color' => 'blue'
														),
														'delete_all_rows' => array(
															'value' => __('Xóa liên kết được chọn', $this->the_plugin->localizationName),
...
<?php
		...
		public function getHitsById( $itemid=array() ) {
			...
			foreach ($itemid as $id) {
				...
				$ret["$id"] = 0;
				$phrase = $wpdb->get_var( "SELECT phrase from " . $wpdb->prefix . "bbit_link_builder WHERE 1=1 and id=" . ( $id ) . ";" );
				if (!is_null($phrase)) {
		...
		public function ajax_request()
		{
			...
			die( json_encode(array(
				'status' => 'valid',
				'data'	=> $wpdb->get_row( "SELECT * from " . $wpdb->prefix . "bbit_link_builder WHERE 1=1 and id=" . ( $request['itemid'] ) . ";" )
			)) );
		}
...
// Initialize the bbitLinkBuilder class
$bbitLinkBuilder = bbitLinkBuilder::getInstance();