...
if (class_exists('bbitTwitterCards') != true) {
    class bbitTwitterCards
    {
        /*
        ...
        */
		...
		private static $prefixFields = 'bbit_twc_';
...
Audio: AAC, Low Complexity Profile (LC)', $this->localizationName)
		...
		public function build_options($pms=array()) {
			...
			switch ($page) {
				...
				case 'post':
					$post_meta = get_post_meta( $post_id, 'bbit_meta', true );
					$options = $post_meta;
					break;
				...
				case 'post-app':
					// $options = $this->plugin_settings;
					$post_meta = get_post_meta( $post_id, 'bbit_meta', true );
					// if ( is_array($post_meta) && !empty($post_meta) && isset($post_meta['bbit_twc_app_description']) )
						$options = $post_meta;
					break;
			}
		...
		public function save_meta($pms=array()) {
			...
			$card_type = isset($_REQUEST['bbit_twc_post_cardtype']) ? strtolower($_REQUEST['bbit_twc_post_cardtype']) : 'none';
			$app_card_type = isset($_REQUEST['bbit_twc_app_isactive']) ? strtolower($_REQUEST['bbit_twc_app_isactive']) : 'no';
			$thumbsize = isset($_REQUEST['bbit_twc_post_thumbsize']) ? $_REQUEST['bbit_twc_post_thumbsize'] : 'none';
			...
			$__options = array();
			$__options['bbit_twc_post_cardtype'] = $card_type;
			$__options['bbit_twc_app_isactive'] = $app_card_type;
			$__options['bbit_twc_post_thumbsize'] = $thumbsize;
		...
		public function get_frontend_meta($meta=array(), $page='') {
			...
			switch ($page) {
				case 'home':
					$card_type = isset($meta['bbit_twc_home_type']) ? $meta['bbit_twc_home_type'] : '';
					$app_card_type = isset($meta['bbit_twc_home_app']) ? $meta['bbit_twc_home_app'] : '';
					break;

				case 'post':
					$card_type = isset($meta['bbit_twc_post_cardtype']) ? $meta['bbit_twc_post_cardtype'] : '';
					$app_card_type = isset($meta['bbit_twc_app_isactive']) ? $meta['bbit_twc_app_isactive'] : '';
					break;
			}
			...
			if ( isset($opt['bbit_twc_site_app']) && !empty($opt['bbit_twc_site_app']) && $opt['bbit_twc_site_app']=='yes' ) {
			...
			$__options[ 'twitter:site' ] = isset($opt['bbit_twc_website_account']) ? $this->tag_special_firstchar($opt['bbit_twc_website_account']) : '';
			$__options[ 'twitter:site:id' ] = isset($opt['bbit_twc_website_account_id']) ? $opt['bbit_twc_website_account_id'] : '';
			$__options[ 'twitter:creator' ] = isset($opt['bbit_twc_creator_account']) ? $this->tag_special_firstchar($opt['bbit_twc_creator_account']) : '';
			$__options[ 'twitter:creator:id' ] = isset($opt['bbit_twc_creator_account_id']) ? $opt['bbit_twc_creator_account_id'] : '';
...
// Initialize the bbitTwitterCards class
//$bbitTwitterCards = new bbitTwitterCards();
