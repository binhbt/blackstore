<?php
/*
* Define class bbitSocialSharing
...
if (class_exists('bbitSocialSharing') != true) {
	class bbitSocialSharing
	{
		/*
		...
		*/
		...
		public function the_styles() {
			if( !wp_style_is('bbit_socialshare_css') ) {
				wp_enqueue_style( 'bbit_socialshare_css' , $this->module_folder . '/social_sharing.css' );
			}
		}
		public function the_scripts() {
			...
			if( !wp_script_is('bbit_socialshare_js') ) {
				wp_enqueue_script( 'bbit_socialshare_js' , $this->module_folder . '/social_sharing.js', array(
					'jquery'
				) );
				wp_localize_script( 'bbit_socialshare_js', 'bbitSocialSharing_ajaxurl', admin_url('admin-ajax.php') );
			}
		}
...
<div class="bbit-form-row">
	...
	<?php
	foreach ($bbit_socialsharing_position as $key => $value){
...
<div class="bbit-form-row">
	...
	<span class="formNote">&nbsp;</span>
	<?php
	foreach ($bbit_socialsharing_margin as $key => $value){
		...
		<label for="<?php echo $field_name.'['.$key.']'; ?>" style="display:inline;float:none;"><?php echo ucfirst(str_replace('_', ' ', $key));?>:</label>
		&nbsp;
		<input type='text' class='' id='<?php echo $field_name.'['.$key.']'; ?>' name='<?php echo $field_name.'['.$key.']'; ?>' value='<?php echo $val; ?>' style="width:100px;">&nbsp;px&nbsp;&nbsp;&nbsp;&nbsp;
		<?php
	} 
	?>
	</div>
		...
		private function set_toolbar_opt( $field_name, $db_meta_name ) {
			...
			$__optArr = array( 
				'btnsize' 		=> $bbit_socialsharing_opt['btnsize'],
				'viewcount' 	=> $bbit_socialsharing_opt['viewcount'],
				'withmore'		=> $bbit_socialsharing_opt['withmore']
			);
...
<div class="bbit-form-row">
	...
	?>
		...
		?>
<div class="bbit-form-row">
	<label>Include/Exclude toolbar on Post, Pages: </label>
	<div class="bbit-form-item large">
	<span class="formNote">&nbsp;</span>
	<?php
	foreach ($bbit_socialsharing_exclude as $key => $value){
		...
		<div class="bbit-form-item large" style="display:inline-block; width:49%; margin-left:0;">
	...
	?>
		...
		private function set_toolbar_optdesign( $field_name, $db_meta_name ) {
			...
			$__optArr = $bbit_socialsharing_design;
...
<div class="bbit-form-row">
	...
	<div class="btn-wrapper">
		<span class="title">Available buttons</span>
		<ul class="btn-available btn-sortable">
		<?php
			if ( !empty($availableBtn) ) {
				foreach ( $availableBtn as $k => $v ) {
		?>
				<li class="block social-btn <?php echo $v; ?>" data-btn="<?php echo $v; ?>"><a class="icon"><span class="title"><?php echo $this->socialNetworks["$v"]['title']; ?></span></a><span class="delete">x</span></li>
		<?php	
				}
			}
		?>
	...
	<div class="btn-wrapper">
		<span class="title">Selected buttons</span>
		...
		private function set_toolbar_contact( $field_name, $db_meta_name ) {
			...
			$__optArr = $bbit_socialsharing_opt['contact'];
		?>
<div class="bbit-form-row">
	...
	?>
		...
		private function getToolbar($toolbarType, $content=false) {
			...
			if ( $content!==false ) {
				...
				// vertical toolbar - always bellow content - moved by js
					...
					$__mark_top = '<span class="bbit-social-content-mark-top"></span>';
					$__mark_bottom = '<span class="bbit-social-content-mark-bottom"></span>';
		...
		private function buildToolbar($toolbarType, $post=null) {
			...
			$ret = '
					<!-- start/ Bbit Plugin - Wordpress Plugin / Social Sharing Toolbar -->
					<div class="bbit-sshare-wrapper ' . $__tbType . ( !empty($cssExtra) ? ' ' . implode(' ', $cssExtra) : '' ) . '" ' . $__params . '>
						<div class="bbit-socialbox-content">'
						. $buttons_list
						. '</div>
					</div>'
					...
					//<![CDATA[
						// bbitSocialSharing.setAjaxUrl( "' . admin_url('admin-ajax.php') . '" );
						var bbitSocialSharing_pms = ' . $toolbarPmsJson . ';
						bbitSocialSharing.build_toolbar( bbitSocialSharing_pms );
					//]]>
					});
					</script>'*/
					. '<!-- end/ Bbit Plugin - Wordpress Plugin / Social Sharing Toolbar -->
			';
			return $ret;
		}
		...
		private function getButtons($toolbarType, $post=null) {
			...
			$sharingButtons = new bbitSocialSharingButtons( $this->the_plugin, $pms );
		...
		private function setToolbarsOptions() {
			...
			return '<div id="bbit-sshare-toolbars-options" style="display: none;" data-options="' . $tblList . '"></div>';
		}
		...
		public function getPostInfo($post, $toolbarType='') {
			...
			if ( $this->the_plugin->is_plugin_active( 'premium-seo-pack/plugin.php' ) ) {
				$__moduleIsActive = get_option('bbit_module_title_meta_format');
			...
			$info = new bbitTitleMetaFormat();

			$info->setPostInfo( $post );
			// $infoFb = new bbitSocialTags(); // facebook
			// $infoTw = new bbitSocialTwitterCards(); // twitter cards
			...
			foreach ( $toolbars as $toolbarType => $cssValue ) {
				if ( $isEnabled["$toolbarType"] ) {
					...
					if ( !empty($bkcolor) ) {
						...
						$tblList["$toolbarType"] = '.bbit-sshare-wrapper.' . $cssValue . ' { background-color: #' . $bkcolor . '; }';
					}
				}
			}
		...
		public function getSocialsData( $website_url='', $itemid=0, $force_refresh_cache=false )
		{
			...
			$the_db_cache = get_post_meta( $itemid, 'bbit_socialsharing_count', true);
			...
			update_post_meta( $itemid, 'bbit_socialsharing_count', $db_cache );
...
// Initialize the bbitSocialSharing class
//$bbitSocialSharing = new bbitSocialSharing();
