	...
	class aaInterfaceTemplates {
		...
		*/
		public function bildThePage ( $options = array(), $alias='', $module=array(), $showForm=true )
		{
			...
			if(count($options) == 0) {
				return __('Please fill whit some options content first!', $bbit->localizationName);
			}
			...
			foreach ( $options as $theBoxs ) {
				...
				foreach ( $theBoxs as $box_id => $box ){
					...
					// container setup
					$html[] = '<div class="bbit-' . ( $box['size'] ) . '">
                        	<div class="bbit-' . ( $box['style'] ) . '">';
					...
					if( $box['header'] == true ) {
						$html[] = '<div class="bbit-panel-header">
							<span class="bbit-panel-title">
							 ...
							 ' . ( $box['toggler'] == true ? '<span class="bbit-panel-toggler"></span>' : '' ) . '
						</div>';
					}

					$html[] = '<div class="bbit-panel-content">';
					if($showForm){
						$html[] = '<form class="bbit-form" id="' . ( $box_id ) . '" action="#save_with_ajax">';
					}
					...
					if(count($box['elements']) > 0){
						...
						foreach ( $box['elements'] as $elm_id => $value ){
							...
							if(!in_array( $value['type'], $noRowElements)){
								...
								$html[] = '<div class="bbit-form-row' . ($this->tabsElements($box, $elm_id)) . '">
									   <label for="' . ( $elm_id ) . '">' . ( isset($value['title']) ? $value['title'] : '' ) . '</label>
									   <div class="bbit-form-item'. ( isset($value['size']) ? " " . $value['size'] : '' ) .'">';
							}
							...
							switch ( $value['type'] ) {
								...
								case 'upload_image_wp':
									...
									$html[] = '<div class="bbit-upload-image-wp-box">';
									$html[] = 	'<a data-previewsize="' . ( $preview_size ) . '" class="upload_image_button_wp bbit-button blue" ' . ( isset($value['force_width']) ? "style='" . ( trim($val) != "" ? 'display:none;' : '' ) . "width:" . ( $value['force_width'] ) . "px;'" : '' ) . ' href="#">' . ( $value['value'] ) . '</a>';
									...
									$html[] = 	'</a>';
									$html[] =	'<div class="bbit-prev-buttons" style="display: ' . ( trim($val) == "" ? 'none' : 'inline-block' ). '">';
									$html[] = 		'<span class="change_image_button_wp bbit-button green">Change Image</span>';
									$html[] = 		'<span class="remove_image_button_wp bbit-button red">Remove Image</span>';
									$html[] =	'</div>';
									$html[] = '</div>';
								break;
								...
								case 'message':
									$html[] = '<div class="bbit-message bbit-' . ( isset($value['status']) ? $value['status'] : '' ) . ' ' . ($this->tabsElements($box, $elm_id)) . '">' . ( $value['html'] ) . '</div>';
								break;
									...
									if(count($value['options']) > 0){
										foreach ($value['options'] as $key => $value){
											$html[] = '<input
												...
												class="bbit-button ' . ( isset($value['color']) ? $value['color'] : '' ) . ' ' . ( isset($value['pos']) ? $value['pos'] : '' ) . ' ' . ( isset($value['action']) ? $value['action'] : '' ) . '"
											/>';
										}
									}
								...
								case 'authorization_button':
									...
									$fb_details = $bbit->getAllSettings('array', 'facebook_planner');
									...
									if( isset($facebook) ) {
											...
											try {
												...
												if(count($user_profile) > 0){
													...
													$loginUrl = $facebook->getLoginUrl(
														...
														'redirect_uri' => admin_url('admin-ajax.php?action=bbit_facebookAuth')
														)
													);
						
													$html[] = '<a href="' . ($loginUrl) . '" style="width: 133px;" class="bbit-button blue">'. (__( 'Authorize this app again', $bbit->localizationName )) .'</a>';
												}
										...
										if( $validAuth == false ) {
											...
											$html[] = '<a href="' . ($loginUrl) . '" style="width: 84px;" type="button" class="bbit-button blue">'. (__( 'Authorizate app', $bbit->localizationName )) .'</a>';
										}
									}
								break;
					...
					// bbit-message use for status message, default it's hidden
					$html[] = '<div class="bbit-message" id="bbit-status-box" style="display:none;"></div>';

					if( $box['buttons'] == true && !is_array($box['buttons']) ) {
						// buttons for each box
						$html[] = '<div class="bbit-button-row">
							<input type="reset" value="' . __('Reset về mặc định', $bbit->localizationName) . '" class="bbit-button gray left" />
							<input type="submit" value="' . __('Lưu thiết lập', $bbit->localizationName) . '" class="bbit-button green bbit-saveOptions" />
						</div>';
					}
					elseif( is_array($box['buttons']) ){
						...
						foreach ( $box['buttons'] as $key => $value ){
							$html[] = '<input type="submit" value="' . ( $value['value'] ) . '" class="bbit-button ' . ( $value['color'] ) . ' ' . ( $value['action'] ) . '" />';
						}
...
?>
		<div id="bbit-wrapper" class="fluid wrapper-bbit">
			...
			<?php
			// show the top menu
			bbitAdminMenu::getInstance()->show_menu( $pluginPage );
			?>

			<!-- Content -->
			<div id="bbit-content">
				<h1 class="bbit-section-headline"></h1>
