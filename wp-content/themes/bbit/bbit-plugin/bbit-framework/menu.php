 ...
 * ===============================+
 *
 * @package		bbitAdminMenu
 * @author		Andrei Dinca
 * @version		1.0
 */
! defined( 'ABSPATH' ) and exit;

if(class_exists('bbitAdminMenu') != true) {
	class bbitAdminMenu {
	    ...
	    */
		...
		private function the_menu_tree()
		{
			if ( isset($this->the_plugin->cfg['modules']['depedencies']['folder_uri'])
				...
				$this->menu_depedencies['depedencies'] = array( 
					...
					'url' => admin_url("admin.php?page=bbit"),
			...
			$this->the_menu['dashboard'] = array( 
				...
				'url' => admin_url("admin.php?page=bbit#dashboard"),
				'folder_uri' => $this->the_plugin->cfg['paths']['freamwork_dir_url'],
				'menu_icon' => 'images/16_dashboard.png'
			);
			
			$this->the_menu['monitoring'] = array( 
				'title' => __( 'Thống Kê', $this->ln ),
				'url' => "#",
				'folder_uri' => $this->the_plugin->cfg['paths']['freamwork_dir_url'],
			...
			$this->the_menu['on_page_optimization'] = array( 
				'title' => __( 'Tối Ưu Hóa ONPage', $this->ln ),
				'url' => "#",
				'folder_uri' => $this->the_plugin->cfg['paths']['freamwork_dir_url'],
					...
					'on_page_optimization' => array(
						...
						'submenu' => array(
							...
							'on_page_optimization_Settings' => array(
								...
								'url' => admin_url("admin.php?page=bbit#on_page_optimization")
							),
						)
					),
			...
			$this->the_menu['off_page_optimization'] = array( 
				'title' => __( 'Tối Ưu Hóa OFFPage', $this->ln ),
				'url' => "#",
				'folder_uri' => $this->the_plugin->cfg['paths']['freamwork_dir_url'],
					...
					'Social_Stats' => array(
						...
						'submenu' => array(
							...
							'Google_Analytics_Settings' => array(
								...
								'url' => admin_url("admin.php?page=bbit#Social_Stats")
							),
						)
					),
			...
			$this->the_menu['advanced_setup'] = array( 
				'title' => __( 'Tính Năng Nâng Cao', $this->ln ),
				'url' => "#",
				'folder_uri' => $this->the_plugin->cfg['paths']['freamwork_dir_url'],
				...
				'submenu' => array(
					...
					'misc--insert-code' => array(
						...
						'url' => admin_url("admin.php?page=bbit#misc#insert-code"),
						'folder_uri' => $this->the_plugin->cfg['paths']['freamwork_dir_url'],
					...
					/*'rich_snippets' => array(
						...
						'url' => admin_url("admin.php?page=bbit#rich_snippets"),
						'folder_uri' => $this->the_plugin->cfg['paths']['freamwork_dir_url'],
						'menu_icon' => 'assets/menu_icon.png'
					),
					...
					'smushit' => array(
						...
						'submenu' => array(
							...
							'Smushit_Settings' => array(
								...
								'url' => admin_url("admin.php?page=bbit#smushit")
							),
						)
					)
			...
			$this->the_menu['general'] = array( 
				'title' => __( 'Cài Đặt Plugin', $this->ln ),
				'url' => "#",
				'folder_uri' => $this->the_plugin->cfg['paths']['freamwork_dir_url'],
				'menu_icon' => 'images/16_generalsettings.png',
				'submenu' => array(
					'modules_manager' => array(
						'title' => __( 'Quản Lí Module', $this->ln ),
					...
					'capabilities' => array(
						...
						'url' => admin_url("admin.php?page=bbit_capabilities"),
						'folder_uri' => $this->the_plugin->cfg['modules']['capabilities']['folder_uri'],
						'menu_icon' => 'assets/menu_icon.png'
					),
					
					'setup_backup' => array(
						'title' => __( 'Cài Đặt / Sao Lưu', $this->ln ),
						'url' => admin_url("admin.php?page=bbit#setup_backup"),
						'folder_uri' => $this->the_plugin->cfg['modules']['setup_backup']['folder_uri'],
						'menu_icon' => 'assets/menu_icon.png'
					),
					
					'server_status' => array(
						'title' => __( 'Trạng Thái Máy Chủ', $this->ln ),
		...
		public function show_menu( $pluginPage='' )
		{
			...
			$html = array();
			// id="bbit-nav-dashboard" 
			$html[] = '<div id="bbit-header">';
			$html[] = 	'<div id="bbit-header-bottom">';
			$html[] = 		'<div id="bbit-topMenu">';
			$html[] = 			'<a href="http://bbit.vn" target="_blank" class="bbit-product-logo">
									<img width="80" heidth="80" src="' . ( $this->the_plugin->cfg['paths']['plugin_dir_url'] ) . 'thumb.png" alt="">
									<h2>Bbit Plugin</h2>
			...
			$html[] = 			'<ul>';
								foreach ($menu as $key => $value) {
									...
									$html[] = '<li id="bbit-nav-' . ( $key ) . '" class="bbit-section-' . ( $key ) . ' ' . ( isset($this->current_menu[0]) && ( $key == $this->current_menu[0] ) ? 'active' : '' ) . '">';
									...
									if( isset($value['submenu']) ){
										...
										foreach ($value['submenu'] as $kk2 => $vv2) {
											...
											$html[] = '<li class="bbit-section-' . ( $kk2 ) . '  ' . ( isset($this->current_menu[1]) && $kk2 == $this->current_menu[1] ? 'active' : '' ) . '" id="bbit-sub-nav-' . ( $kk2 ) . '">';
											$html[] = 	$iconImg;
											$html[] = 	'<a href="' . ( $vv2['url'] ) . '">' . ( $vv2['title'] ) . '</a>'; 
											
											if( isset($vv2['submenu']) ){
												$html[] = 	'<ul class="bbit-sub-sub-menu">';
												foreach ($vv2['submenu'] as $kk3 => $vv3) {
													$html[] = '<li id="bbit-sub-sub-nav-' . ( $kk3 ) . '">';
			...
			$html[] = 	'<a href="http://bbit.vn" class="bbit-aateam-logo">Bbit</a>';
			
			$html[] = '</div>';
			
			$html[] = '<script>
			(function($) {
				var bbitMenu = $("#bbit-topMenu");

				bbitMenu.on("click", "a", function(e){
					
					var that = $(this),
						href = that.attr("href");

					if( href == "javascript: void(0)" ){
						var current_open = bbitMenu.find("li.active");
						current_open.find(".bbit-sub-menu").slideUp(350);
						current_open.removeClass("active");
					}
					
					that.parent("li").eq(0).find(".bbit-sub-menu").slideDown(350);
