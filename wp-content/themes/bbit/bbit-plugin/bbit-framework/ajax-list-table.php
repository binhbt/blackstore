 ...
 * ===============================+
 *
 * @package		bbitAjaxListTable
 * @author		Andrei Dinca
 * @version		1.0
 */
! defined( 'ABSPATH' ) and exit;

if(class_exists('bbitAjaxListTable') != true) {
	class bbitAjaxListTable {
        ...
        */
        public function __construct( $parent )
        {
			...
			add_action('wp_ajax_bbitAjaxList', array( $this, 'request' ));
	    ...
	    */
		public function setup( $options=array() )
		{
			...
			//unset($_SESSION['bbitListTable']); // debug

			// check if set, if not, reset
			if ( isset($options['requestFrom']) && $options['requestFrom'] == 'ajax' ) ;
			else {

				$keepvar = isset($_SESSION['bbitListTable']['keepvar']) ? $_SESSION['bbitListTable']['keepvar'] : '';
				$sess = isset($_SESSION['bbitListTable'][$this->opt['id']]['params']) ? $_SESSION['bbitListTable'][$this->opt['id']]['params'] : array();
				...
				if ( isset($keepvar) && isset($keepvar['paged']) ) {
					...
					unset( $keepvar['paged'] );
					$_SESSION['bbitListTable']['keepvar'] = $keepvar;
				}

			}
			$_SESSION['bbitListTable'][$this->opt['id']] = $options;
		...
		public function request()
		{
			...
			if( $request['sub_action'] == 'post_per_page' ){
				...
				if( $new_post_per_page == 'all' ){
					$_SESSION['bbitListTable'][$request['ajax_id']]['params']['posts_per_page'] = '-1';
				}
				elseif( (int)$new_post_per_page == 0 ){
					$_SESSION['bbitListTable'][$request['ajax_id']]['params']['posts_per_page'] = $this->opt['items_per_page'];
				}
				else{
					$_SESSION['bbitListTable'][$request['ajax_id']]['params']['posts_per_page'] = $new_post_per_page;
				}

				// reset the paged as well
				$_SESSION['bbitListTable'][$request['ajax_id']]['params']['paged'] = 1;
			}
			...
			if( $request['sub_action'] == 'paged' ){
				...
				$_SESSION['bbitListTable'][$request['ajax_id']]['params']['paged'] = $new_paged;
			}
			...
			if( $request['sub_action'] == 'post_type' ){
				...
				$_SESSION['bbitListTable'][$request['ajax_id']]['params']['post_type'] = $new_post_type;
			...
			if( $request['sub_action'] == 'post_status' ){
				...
				$_SESSION['bbitListTable'][$request['ajax_id']]['params']['post_status'] = $new_post_status;
			...
			if( $request['sub_action'] == 'search' ){
				...
				$_SESSION['bbitListTable'][$request['ajax_id']]['params']['search_text'] = $search_text;
			...
			$_SESSION['bbitListTable'][$request['ajax_id']]['requestFrom'] = 'ajax';

			$this->setup( $_SESSION['bbitListTable'][$request['ajax_id']] );
			$this->print_html();
			...
			die( json_encode(array(
				'status' 	=> 'valid',
				'html'		=> $html
				//,'sess'		=> $_SESSION['bbitListTable'][$request['ajax_id']]['params']
			)) );
		}
	    ...
	    */
		public function get_items()
		{
			...
			$ses = isset($_SESSION['bbitListTable'][$this->opt['id']]['params']) ? $_SESSION['bbitListTable'][$this->opt['id']]['params'] : array();
			...
			if ( $_SESSION['bbitListTable'][$this->opt['id']]['id'] == 'bbitSmushit' ) { // smushit
			...
			if( trim($this->opt["custom_table"]) != ""){
				...
				if( $this->opt["custom_table"] == 'bbit_serp_reporter' ) {
					$result_query = str_replace('1=1 limit', " 1=1 ORDER BY focus_keyword desc limit ", $result_query);
			    	if (isset($_SESSION['bbit_serp']['search_engine']) && !empty($_SESSION['bbit_serp']['search_engine']) && $_SESSION['bbit_serp']['search_engine']!='--all--') {
			    		$myQuery = str_replace("1=1 ", " 1=1 and search_engine='".$_SESSION['bbit_serp']['search_engine']."' ", $myQuery);
			    		$result_query = str_replace("1=1 ", " 1=1 and search_engine='".$_SESSION['bbit_serp']['search_engine']."' ", $result_query);
			    	}
				}
			    ...
			    foreach ($query as $key => $myrow){
			    	if( $this->opt["custom_table"] == 'bbit_monitor_404' ) {
					...
					else if( $this->opt["custom_table"] == 'bbit_web_directories' ) {
						$pages[$myrow['id']] = array(
							'id' 					=> $myrow['id'],
					...
					else if( $this->opt["custom_table"] == 'bbit_link_builder' ) {
						$pages[$myrow['id']] = array(
							'id' 			=> $myrow['id'],
							'url' 			=> $myrow['url'],
				    		'phrase' 		=> $myrow['phrase'],
					...
					else if( $this->opt["custom_table"] == 'bbit_proxy_lists' ) {
						$pages[$myrow['id']] = array(
							'id' 				=> $myrow['id'],
							'ip' 				=> $myrow['ip'],
				    		'port' 				=> $myrow['port'],
					...
					else if( $this->opt["custom_table"] == 'bbit_link_redirect' ) {
						$pages[$myrow['id']] = array(
							'id' 				=> $myrow['id'],
							'url' 				=> $myrow['url'],
					...
					else if( $this->opt["custom_table"] == 'bbit_serp_reporter' ) {
						$pages[$myrow['id']] = array(
							'id' 					=> $myrow['id'],
			    	...
			    	else if( $this->opt["custom_table"] == 'bbit_post_planner_cron' ) {
			    		$pages[$myrow['id']] = array(
							'id' 					=> $myrow['id'],
		...
		public function search_posts_where( $where ) {
			...
			if( is_admin() ) {
				$ses = $_SESSION['bbitListTable'][$this->opt['id']]['params'];
		...
		public function post_statuses_filter()
		{
			...
			$html[] = 		'<ul class="subsubsub bbit-post_status-list">';
		...
		private function get_pagination()
		{
			...
			if( $this->opt['show_pagination'] ){
				$html[] = 	'<div class="bbit-list-table-right-col">';


				$html[] = 		'<div class="bbit-box-show-per-pages">';
				$html[] = 			'<select name="bbit-post-per-page" id="bbit-post-per-page" class="bbit-post-per-page">';
				...
				$html[] = 				'</option>';
				$html[] =			'</select>';
				$html[] = 			'<label for="bbit-post-per-page" style="width:57px">' . __('per pages', $this->the_plugin->localizationName) . '</label>';
				$html[] = 		'</div>';

				$html[] = 		'<div class="bbit-list-table-pagination tablenav">';

				$html[] = 			'<div class="tablenav-pages">';
				$html[] = 				'<span class="displaying-num">' . ( $this->items_nr ) . ' ' . __('items', $this->the_plugin->localizationName) . '</span>';
				if( $total_pages > 1 ){

				$html[] = 				'<span class="pagination-links"><a class="first-page ' . ( $paged == 1 ? 'disabled' : '' ) . ' bbit-jump-page" title="' . __('Go to the first page', $this->the_plugin->localizationName) . '" href="#paged=1">«</a>';
					$html[] = 				'<a class="prev-page ' . ( $paged == 1 ? 'disabled' : '' ) . ' bbit-jump-page" title="' . __('Go to the previous page', $this->the_plugin->localizationName) . '" href="#paged=' . ( $paged > 2 ? ($paged - 1) : 1 ) . '">‹</a>';
					...
					$html[] = 				'<a class="next-page ' . ( ( $paged == ($total_pages)) ? 'disabled' : '' ) . ' bbit-jump-page" title="' . __('Go to the next page', $this->the_plugin->localizationName) . '" href="#paged=' . ( $paged < $total_pages ? $paged + 1 : $total_pages ) . '">›</a>';
					$html[] = 				'<a class="last-page ' . ( $paged ==  ($total_pages - 1) ? 'disabled' : '' ) . ' bbit-jump-page" title="' . __('Go to the last page', $this->the_plugin->localizationName	) . '" href="#paged=' . ( $total_pages ) . '">»</a></span>';
				}
				$html[] = 			'</div>';
				$html[] = 		'</div>';
		...
		public function print_header()
		{
			...
			$html[] = '<div id="bbit-list-table-header">';

			if( trim($this->opt["custom_table"]) == ""){
				$html[] = '<div class="bbit-list-table-left-col">';

				if ( $_SESSION['bbitListTable'][$this->opt['id']]['id'] != 'bbitSmushit' ) { // if NOT smushit

				$html[] = 		'<select name="bbit-filter-post_type" class="bbit-filter-post_type">';
				...
				if( $this->opt['search_box'] ){
					...
					$html[] = 	'<div class="bbit-list-table-right-col">';
					$html[] = 		'<div class="bbit-list-table-search-box">';
					$html[] = 			'<input type="text" name="bbit-search-text" id="bbit-search-text" value="'.($search_text).'" class="'.($search_text!='' ? 'search-highlight' : '').'" >';
					$html[] = 			'<input type="button" name="bbit-search-btn" id="bbit-search-btn" class="button" value="' . __('Search Posts', $this->the_plugin->localizationName) . '">';
					$html[] = 		'</div>';
					$html[] = 	'</div>';
				}
				...
				/*if ( isset($this->opt['notices']['default']) ) {
					if ( isset($this->opt['notices']['default_clause'])
						...
						&& count($this->items) <= 0 ) {
						$show_notice = true;
						$html[] = '<div class="bbit-list-table-left-col">' . $this->opt['notices']['default'] . '</div>';
					}
				}*/
				
				if ( !$show_notice )
					$html[] = '<div class="bbit-list-table-left-col">&nbsp;</div>';
			}
			...
			if( trim($this->opt["custom_table"]) == "" || $this->opt["custom_table_force_action"]){
				...
				if( isset($this->opt['mass_actions']) && ($this->opt['mass_actions'] === false) ){
					$html[] = '<div class="bbit-list-table-left-col" style="padding-top: 5px;">&nbsp;</div>';
				}elseif( isset($this->opt['mass_actions']) && count($this->opt['mass_actions']) > 0 ){
					$html[] = '<div class="bbit-list-table-left-col" style="padding-top: 5px;">&nbsp;';

					foreach ($this->opt['mass_actions'] as $key => $value){
						$html[] = 	'<input type="button" value="' . ( $value['value'] ) . '" id="bbit-' . ( $value['action'] ) . '" class="bbit-button ' . ( $value['color'] ) . '">';
					}
					$html[] = '</div>';
				}else{
					$html[] = '<div class="bbit-list-table-left-col" style="padding-top: 5px;">&nbsp;';
					$html[] = 	'<input type="button" value="' . __('Auto detect focus keyword for All', $this->the_plugin->localizationName) . '" id="bbit-all-auto-detect-kw" class="bbit-button blue">';
					$html[] = 	'<input type="button" value="' . __('Optimize All', $this->the_plugin->localizationName) . '" id="bbit-all-optimize" class="bbit-button blue">';
					$html[] = '</div>';
				}
				
				if( $this->opt['id'] == 'bbitPageOptimization' ){
					$html[] = '<div id="bbit-inline-editpost-boxtpl" style="display: none;">';
		...
		public function print_main_table( $items )
		{
			...
			$html[] = 	'<table class="bbit-table" style="border: none;border-bottom: 1px solid #dadada;">';
			$html[] = 		'<thead>';
			$html[] = 			'<tr>';
  
			foreach ($this->opt['columns'] as $key => $value){
				if( $value['th'] == 'checkbox' ){
					$html[] = '<th class="checkbox-column" width="20"><input type="checkbox" id="bbit-item-check-all" checked></th>';
				}
				else{
			...
			foreach ($this->items as $post){
				if( isset($post->ID) ){
					$item_data = array(
						'score' 	=> get_post_meta( $post->ID, 'bbit_score', true )
					);
				}
				...
				foreach ($this->opt['columns'] as $key => $value){
					...
					if( $value['td'] == 'checkbox' ){
						$html[] = '<input type="checkbox" class="bbit-item-checkbox" name="bbit-item-checkbox-' . ( isset($post->ID) ? $post->ID : $post['id'] ) . '" checked>';
					}
					elseif( $value['td'] == '%score%' ){
						...
						$html[] = '<div class="bbit-progress">';
						$html[] = 	'<div class="bbit-progress-bar ' . ( $size_class ) . '" style="width:' . ( $score ) . '%"></div>';
						$html[] = 	'<div class="bbit-progress-score">' . ( $item_data['score'] ) . '%</div>';
						$html[] = '</div>';
					}
					elseif( $value['td'] == '%focus_keyword%' ){
						$focus_kw = get_post_meta( $post->ID, 'bbit_kw', true );
						$html[] = '<div class="bbit-focus-kw-box">';
						$html[] = 	'<input type="text" class="bbit-text-field-kw" id="bbit-focus-kw-' . ( $post->ID ) . '" value="' . ( $focus_kw ) . '" />';
						$html[] = 	'<input type="button" class="bbit-auto-detect-kw-btn bbit-button gray" value="' . __('Auto detect', $this->the_plugin->localizationName) . '" />';
						/*$html[] = 	'<a class="bbit-button green bbit-suggest-kw-btn" href="#">
					...
					elseif( $value['td'] == '%seo_report%' ){
						$html[] = '<a class="bbit-button green bbit-seo-report-btn" href="#" data-itemid="' . ( $post->ID ) . '">
                                        <img src="' . ( $this->the_plugin->cfg['paths']['freamwork_dir_url'] ) . 'images/light.png">
					...
					elseif( $value['td'] == '%title%' ){
						$html[] = '<input type="hidden" id="bbit-item-title-' . ( $post->ID ) . '" value="' . ( str_replace('"', "'", $post->post_title) ) . '" />';
						...
						if ( $post->post_status == 'inherit' && $post->post_type == 'attachment' ) { // media image file
							...
							$html[] = '</a>';
							$html[] = '
							<span class="bbit-inline-row-actions show" id="bbit-inline-row-actions-' . ( $post->ID ) . '">
					...
					elseif( $value['td'] == '%title_and_actions%' ){
						...
						$html[] = '
						<div id="bbit-inline-row-data-' . ( $post->ID ) . '" class="hide" style="display: none;">
							'.$__row_actions.'
						</div>
						';
					}
					...
					elseif( $value['td'] == '%button%' ){
						...
						$html[] = 	'<input type="button" value="' . ( $value['option']['value'] ) . '" class="bbit-button ' . ( $value['option']['color'] ) . ' bbit-' . ( $value['option']['action'] ) . '">';
					}
					elseif( $value['td'] == '%button_publish%' ){
						$value['option']['color'] = isset($value['option']['color']) ? $value['option']['color'] : 'gray';
						$html[] = 	'<input type="button" value="' . ( $post['publish']=='Y' ? $value['option']['value'] : $value['option']['value_change'] ) . '" class="bbit-button ' . ( $value['option']['color'] ) . ' bbit-' . ( $value['option']['action'] ) . '">';
					}
					...
					elseif( $value['td'] == '%referrers%' ){
						$html[] = (trim($post['referrers']) != "" ? '<a href="#referrers" class="bbit-button gray bbit-btn-referrers-lightbox" data-itemid="' . ( $post['id'] ) . '">' . ( __('Show All', $this->the_plugin->localizationName) ) . '</a>' : '-');
					}
					elseif( $value['td'] == '%user_agents%' ){
						$html[] = (trim($post['user_agents']) != "" ? '<a href="#user_agents" class="bbit-button gray bbit-btn-user_agents-lightbox" data-itemid="' . ( $post['id'] ) . '">' . ( __('Show All', $this->the_plugin->localizationName) ) . '</a>' : '-');
					}
					elseif( $value['td'] == '%last_date%' ){
						$html[] = '<i>' . ( $post['data'] ) . '</i>';
					}
					
					if( $this->opt['id'] == 'bbitSmushit' ){
						...
						if( $value['td'] == '%thumbnail%' ){
							...
							//$html[] = '<div class="bbit-message">';
							//$html[] = 	'<span class="bbit-smushit-loading"></span>';
			
							// retrieve the existing value(s) for this meta field. This returns an array
							$meta_new = wp_get_attachment_metadata( $id );

							if ( isset($meta_new['bbit_smushit']) && !empty($meta_new['bbit_smushit']) ) {
			
								$msg = (array) $this->the_plugin->smushit_show_sizes_msg_details( $meta_new ); $__msg = array();
								if ( isset($meta_new['bbit_smushit_errors']) && ( (int) $meta_new['bbit_smushit_errors'] ) > 0 ) {
									$status = 'invalid';
									$msg_cssClass = 'bbit-error';
									$__msg = array( __('errors occured during smushit operation!', $this->the_plugin->localizationName) );
								} else {
									$status = 'valid';
									$msg_cssClass = 'bbit-success';
								}
								$msg = implode('<br />', array_merge($__msg, $msg));
								
								$html[] = '<div id="' . ('bbit-smushit-resp-'.$id) . '" class="bbit-message ' . $msg_cssClass . '">' . $msg . '</div><br />';
							} else {
								
								$html[] = '<div id="' . ('bbit-smushit-resp-'.$id) . '" class="bbit-message bbit-info">' . __( 'not processed!', $this->the_plugin->localizationName ) . '</div><br />';
							}
							//$html[] = '</div>';
					...
					if( $this->opt['id'] == 'bbitPageSpeed' ){
						if( $value['td'] == '%mobile_score%' ){
							$mobile = get_post_meta( $post->ID, 'bbit_mobile_pagespeed', true ); 
							...
							if( isset($mobile['score']) ){
								...
								$html[] = '<div class="bbit-progress" style="margin-right:4px">';
								$html[] = 	'<div class="bbit-progress-bar ' . ( $size_class ) . '" style="width:' . ( $score ) . '%"></div>';
								$html[] = '</div>';
							}else{
						...
						if( $value['td'] == '%desktop_score%' ){
							$desktop = get_post_meta( $post->ID, 'bbit_desktop_pagespeed', true ); 
					...
					if( $this->opt['id'] == 'bbitLinkBuilder' ){
						if( $value['td'] == '%builder_phrase%' ){
							...
							$html[] = (1==1 ? '<a href="#url_attributes" class="bbit-button gray bbit-btn-url-attributes-lightbox" data-itemid="' . ( $post['id'] ) . '">' . ( __('Show All', $this->the_plugin->localizationName) ) . '</a>' : '-');
						}
					}
					
					if( $this->opt['id'] == 'bbitLinkRedirect' ){
					...
					if( $this->opt['id'] == 'bbitSocialStats' ){
						...
						if( $value['td'] == '%ss_facebook%' ){
							$html[] = '<div class="bbit-social-status" style="color: #3c5b9b">';
							$html[] = 	'<img src="' . ( $dashboard_module_url ) . 'assets/stats/facebook-icon.png" class="bbit-lists-icon">';
							...
							/*
							$html[] = '<div class="bbit-social-status" style="color: #3c5b9b">';
							$html[] = 	'<img src="' . ( $dashboard_module_url ) . 'assets/stats/facebook-like-icon.png" class="bbit-lists-icon">';
							...
							$html[] = '<div class="bbit-social-status" style="color: #3c5b9b">';
							$html[] = 	'<img src="' . ( $dashboard_module_url ) . 'assets/stats/facebook-comments-icon.png" class="bbit-lists-icon">';
						...
						elseif( $value['td'] == '%ss_stumbleupon%' ){
							$html[] = '<div class="bbit-social-status" style="color: #3fbd46">';
							$html[] = 	'<img src="' . ( $dashboard_module_url ) . 'assets/stats/stumbleupon-icon.png" class="bbit-lists-icon">';
						...
						elseif( $value['td'] == '%ss_twitter%' ){
							$html[] = '<div class="bbit-social-status" style="color: #00aced">';
							$html[] = 	'<img src="' . ( $dashboard_module_url ) . 'assets/stats/twitter-icon.png" class="bbit-lists-icon">';
						...
						elseif( $value['td'] == '%ss_google%' ){
							$html[] = '<div class="bbit-social-status" style="color: #d23e2b">';
							$html[] = 	'<img src="' . ( $dashboard_module_url ) . 'assets/stats/google-icon.png" class="bbit-lists-icon">';
						...
						elseif( $value['td'] == '%ss_digg%' ){
							$html[] = '<div class="bbit-social-status" style="color: #2c2c2c">';
							$html[] = 	'<img src="' . ( $dashboard_module_url ) . 'assets/stats/delicious-icon.png" class="bbit-lists-icon">';
						...
						elseif( $value['td'] == '%ss_pinterest%' ){
							$html[] = '<div class="bbit-social-status" style="color: #ca4638">';
							$html[] = 	'<img src="' . ( $dashboard_module_url ) . 'assets/stats/pinterest-icon.png" class="bbit-lists-icon">';
						...
						elseif( $value['td'] == '%ss_linkedin%' ){
							$html[] = '<div class="bbit-social-status" style="color: #007ab9">';
							$html[] = 	'<img src="' . ( $dashboard_module_url ) . 'assets/stats/linkedin-icon.png" class="bbit-lists-icon">';
					...
					if( $this->opt['id'] == 'bbitWebDirectories' ){
						...
						elseif( $value['td'] == '%submit_btn%' ){
							$html[] = '<a href="' . ( $post['submit_url'] ) . '" target="_blank" class="bbit-button blue bbit-btn-submit-website" data-itemid="' . ( $post['id'] ) . '">' . ( __('Submit website', $this->the_plugin->localizationName) ) . '</a>';
						}

						elseif( $value['td'] == '%submit_status%' ){
							// never submited / $post['status'] = 2;
							$html_status = '<div class="bbit-message" style="padding: 5px;">' . ( __('Never submit', $this->the_plugin->localizationName) ) . '</div>';
							if( $post['status'] == 2 ){
								$html_status = '<div class="bbit-message bbit-info" style="padding: 5px;background-image: none;">' . ( __('Submit in progress', $this->the_plugin->localizationName) ) . '</div>';
							}
							elseif( $post['status'] == 3 ){
								$html_status = '<div class="bbit-message bbit-error" style="padding: 5px;background-image: none;">' . ( __('Error on submit', $this->the_plugin->localizationName) ) . '</div>';
							}
							elseif( $post['status'] == 1 ){
								$html_status = '<div class="bbit-message bbit-success" style="padding: 5px;background-image: none;">' . ( __('Submit successfully', $this->the_plugin->localizationName) ) . '</div>';
							}
					...
					if( $this->opt['id'] == 'bbitPageHTMLValidation' ){

						// get html verify data
						$html_verify_details = get_post_meta( $post->ID, 'bbit_w3c_validation', true );
						...
						elseif( $value['td'] == '%view_full_report%' ){
							$html[] = '<a target="_blank" href="' . ( 'http://validator.w3.org/check?uri=' . get_permalink( $post->ID ) ) . '" class="bbit-button gray">' . ( __('View report', $this->the_plugin->localizationName) ) . '</a>';
						}
					}
					
					if( $this->opt['id'] == 'bbitSERPKeywords' ){
						$rank_data = $post;
						...
						elseif( $value['td'] == '%serp_location%' ){
							...
							if( isset($rank_data) && is_array($rank_data) && count($rank_data) > 0 ){
								...
								$html[] = 					'<td width="57">';
								if( $current_pos==999 ){
									$html[] = 					'<div class="bbit-rank-container-block-extra">' . ( $__icon_not100 ) . '</div>';
								}else{
								
									$html[] = 					'<div class="bbit-rank-container-block">';
								...
								$html[] = 					'</td>';
								$html[] = 					'<td width="35"><div class="bbit-rank-container-block-extra">' . ( $best_pos==999 ? $__icon_not100 : '#'.$best_pos ) . '</div></td>';
								$html[] = 					'<td><div class="bbit-rank-container-block-extra">' . ( $worst_pos==999 ? $__icon_not100 : '#'.$worst_pos ) . '</div></td>';
								$html[] = 			'</tr>';
								$html[] = 		'</tbody>';
								$html[] = 	'</table>';
								$html[] = '</div>';
					...
					if( $this->opt['id'] == 'bbitFacebookPlanner' ){
						...
						elseif( $value['td'] == '%post_name%' ){
							...
							$html[] = '<input type="hidden" id="bbit-item-title-' . ( $post['id'] ) . '" value="' . ( str_replace('"', "'", $__postInfo->post_title) ) . '" />';
							...
							$pg = get_option('bbit_fb_planner_user_pages');
							if(trim($pg) != ""){
								$pg = @json_decode($pg);
							}
		...
		public function print_html()
		{
			...
			$html[] = '<input type="hidden" class="bbit-ajax-list-table-id" value="' . ( $this->opt['id'] ) . '" />';
		...
		public function get_page_social_stats( $postID, $website_url='' )
		{
			...
			$the_db_cache = get_post_meta( $postID, '_bbit_social_stats', true );
			...
			if ( !empty($this->the_plugin->charset) )
				$html = bbitphpQuery::newDocumentHTML( $go_data, $this->the_plugin->charset );
			else
				$html = bbitphpQuery::newDocumentHTML( $go_data );
			...
			update_post_meta( $postID, '_bbit_social_stats', $db_cache );
