...
if(class_exists('bbit') != true) {
	class bbit {
		...
		function __construct($here = __FILE__)
		{
			...
			$this->plugin_hash = get_option('bbit_hash');
			...
			$this->buildConfigParams('paths', array(
				...
				'plugin_dir_url' => str_replace('bbit-framework/', '', get_template_directory_uri( (__FILE__)  ).'/bbit-plugin/'),

				// http://codex.wordpress.org/Function_Reference/plugin_dir_path
				'plugin_dir_path' => str_replace('bbit-framework/', '', get_template_directory( (__FILE__) )).'/bbit-plugin/'
			));
			...
			$this->buildConfigParams('paths', array(
				'freamwork_dir_url' => $this->cfg['paths']['plugin_dir_url'] . 'bbit-framework/',
				'freamwork_dir_path' => $this->cfg['paths']['plugin_dir_path'] . 'bbit-framework/'
			));
			...
			require_once( $this->cfg['paths']['plugin_dir_path'] . 'bbit-framework/menu.php' );
			
			// Run the plugins section load method
			add_action('wp_ajax_bbitLoadSection', array( &$this, 'load_section' ));
			
			// Plugin Depedencies Verification!
			if (get_option('bbit_depedencies_is_valid', false)) {
				...
				if (get_option('bbit_depedencies_do_activation_redirect', false)) {
			...
			add_action('wp_ajax_bbitSaveOptions', array( &$this, 'save_options' ));

			// Run the plugins section options save method
			add_action('wp_ajax_bbitModuleChangeStatus', array( &$this, 'module_change_status' ));
			
			// Run the plugins section options save method
			add_action('wp_ajax_bbitModuleChangeStatus_bulk_rows', array( &$this, 'module_bulk_change_status' ));

			// Run the plugins section options save method
			add_action('wp_ajax_bbitInstallDefaultOptions', array( &$this, 'install_default_options' ));

			// W3CValidate helper
			add_action('wp_ajax_bbitW3CValidate', array( &$this, 'bbitW3CValidate' ));

			// W3CValidate helper
			add_action('wp_ajax_bbitUpload', array( &$this, 'upload_file' ));
			add_action('wp_ajax_bbitWPMediaUploadImage', array( &$this, 'wp_media_upload_image' ));
			
			require_once( $this->cfg['paths']['scripts_dir_path'] . '/utf8/utf8.php' );
			$this->utf8 = bbitUtf8::getInstance();
			
			// admin ajax action
			require_once( $this->cfg['paths']['plugin_dir_path'] . 'bbit-framework/utils/action_admin_ajax.php' );
			new bbitActionAdminAjax( $this );
			
			// import seo data
			require_once( $this->cfg['paths']['plugin_dir_path'] . 'bbit-framework/utils/import_seodata.php' );
			new bbitImportSeoData( $this );
			
			add_action('admin_init', array($this, 'plugin_redirect'));
			
			if( $this->debug == true ){
				add_action('wp_footer', array($this, 'print_bbit_usages') );
				add_action('admin_footer', array($this, 'print_bbit_usages') );
			}
			
			require_once( $this->cfg['paths']['plugin_dir_path'] . 'bbit-framework/ajax-list-table.php' );
			new bbitAjaxListTable( $this );
			
			// shortcodes
			require_once($this->cfg['paths']['plugin_dir_path'] . 'bbit-framework/shortcodes/shortcodes.init.php');
			...
			if ( !is_admin() ) {
				if ( isset($_POST['isbbitreq']) && in_array( $_POST['isbbitreq'], array('tax', 'post') ) ) {
					if ( $_POST['isbbitreq'] == 'post' )
						add_filter( 'the_content', array( $this, 'mark_content' ), 0, 1 );
					else if ( $_POST['isbbitreq'] == 'tax' ) {
		...
		{
			return '<div id="bbit-content-mark">' . $content . '</div>';
		}
		...
		public function getPageContent( $post=null, $oldcontent='', $istax=false ) 
		{
			...
			$resp = wp_remote_post( $url, array(
				...
				'blocking' => true,
				'headers' => array(),
				'body' => array( 'isbbitreq' => ( $istax ? 'tax' : 'post' ) ),
				'cookies' => array()
			));
			...
			//$pattern = "/\[bbitmark\].*\[\/bbitmark\]/imu";
			//$ret = preg_match($pattern, $content, $matches);
  
			// php query class
			require_once( $this->cfg['paths']['scripts_dir_path'] . '/php-query/php-query.php' );
			if ( !empty($this->charset) )
				$doc = bbitphpQuery::newDocument( $content, $this->charset );
			else
				$doc = bbitphpQuery::newDocument( $content );
			
			$content = bbitPQ('#bbit-content-mark');
		...
		<div id="message" class="updated wc-connect" style="border-left: 0px!important; padding: 0!important">
		...
		public function plugin_redirect() {
			if (get_option('bbit_do_activation_redirect', false)) {
				delete_option('bbit_do_activation_redirect');
				wp_redirect( get_admin_url() . 'admin.php?page=bbit' );
			}
		}

		public function activate()
		{
			add_option('bbit_do_activation_redirect', true);
			add_option('bbit_depedencies_is_valid', true);
			add_option('bbit_depedencies_do_activation_redirect', true);
		}

		public function get_plugin_status ()
		{
			return $this->v->isReg( get_option('bbit_hash') );
		}
		...
		public function save_options ()
		{
			...
			if ( $save_id == 'bbit_local_seo' && isset($savingOptionsArr['slug']) ) {
				$savingOptionsArr['slug'] = sanitize_title( $savingOptionsArr['slug'] );
			}
			if ( $save_id == 'bbit_socialsharing' /*&& isset($savingOptionsArr['toolbar']) && $savingOptionsArr['toolbar']!='none'*/ ) {
				$__old_saving = get_option('bbit_socialsharing', true);
			...
			if( isset($tryed_module) && count($tryed_module) > 0 ){
				...
				if(trim($options) != "") {
					...
					if(class_exists('aaInterfaceTemplates') != true) {
						...
						if(trim($html) != "") {
							$headline = $tryed_module[$request['section']]['menu']['title'] . "<span class='bbit-section-info'>" . ( $tryed_module[$request['section']]['description'] ) . "</span>";
		...
		*/
		...
		public function admin_load_styles()
		{
			...
			wp_enqueue_style( 'bbit-bbit-framework-styles', $this->cfg['paths']['freamwork_dir_url'] . 'load-styles.php' );
		...
		*/
				...
				if( isset($module[$alias]["load_in"]['backend']) && is_array($module[$alias]["load_in"]['backend']) && count($module[$alias]["load_in"]['backend']) > 0 ){
					...
					foreach ( $module[$alias]["load_in"]['backend'] as $page ) {
						...
						if(
							...
							( ( $page == '@all' ) || ( $current_url == '/wp-admin/admin.php?page=bbit' ) || ( !empty($page) && $urlfound > 0 ) )
		...
		public function createDashboardPage ()
		{
			...
			//if( $bbit->can_manage('view_seo_dashboard') ){
				add_menu_page(
					__( 'Bbit Plugin - Dashboard'),
					__( 'Bbit Plugin' ),
					'read',
					$this->alias,
		...
		public function getAllSettings( $returnType='array', $only_box='' )
		{
			...
			if( count($results) > 0 ){
				foreach ($results as $key => $value){
					...
					//special case - it's not double serialized!
					if ($value['option_name']=='bbit_taxonomy_seo') {
		...
		*/
						...
						if( is_array($settings[$alias]["load_in"]['backend']) && count($settings[$alias]["load_in"]['backend']) > 0 ){
							...
							foreach ( $settings[$alias]["load_in"]['backend'] as $page ) {
								...
								if(
									// $current_url == '/wp-admin/' . $page ||
									( ( $page == '@all' ) || ( $current_url == '/wp-admin/admin.php?page=bbit' ) || ( !empty($page) && $urlfound > 0 ) )
									|| ( $action == 'bbitLoadSection' && $section == $alias )
									|| substr($action, 0, 3) == 'bbit'
								){
									$need_to_load = true;  
								}
							}
		...
		public function print_bbit_usages()
		{
			$html = array();
			...
			$html[] = '<h1 class="bbit-bench-title">PSP: Benchmark performance</h1>';
			$html[] = '<table class="bbit-bench-log">';
			$html[] = 	'<thead>';
			$html[] = 		'<tr>';
			$html[] = 			'<th>Module</th>';
		...
		*/
		...
		public function bbitW3CValidate()
		{
			require_once( $this->cfg['modules']['W3C_HTMLValidator']['folder_path'] . 'app.class.php' );
			$bbitW3C_HTMLValidator = new bbitW3C_HTMLValidator($this->cfg, $module);
			$bbitW3C_HTMLValidator->validateLink();
		}
	    ...
	    */
		...
		public function get_page_meta( $url='' )
		{
			...
			if( trim($url) != "" ){
				...
				if( trim($html_data) != "" ){
					require_once( $this->cfg['paths']['scripts_dir_path'] . '/php-query/php-query.php' );
					if ( !empty($this->charset) )
						$doc = bbitphpQuery::newDocument( $html_data, $this->charset );
					else
		...
		public function verify_module_status( $module='' ) {
			...
			$mod_active = get_option( 'bbit_module_'.$module );
			if ( $mod_active != 'true' )
		...
		public function edit_post_inline_data( $post_id, $seo=null, $tax=false, $post_content='empty' ) {
			...
			if ( $this->__tax_istax( $tax ) ) { //taxonomy data!
				...
				$bbit_current_taxseo = $this->__tax_get_post_meta( null, $post );
				if ( is_null($bbit_current_taxseo) || !is_array($bbit_current_taxseo) )
					$bbit_current_taxseo = array();

				$post_metas = $this->__tax_get_post_meta( $bbit_current_taxseo, $post, 'bbit_meta' );
			} else {
			...
			$html = array();
			$html[] = '<div class="bbit-post-title">' . $post_title . '</div>';
			$html[] = '<div class="bbit-post-gen-desc">' . $gen_meta_desc . '</div>';
			$html[] = '<div class="bbit-post-gen-keywords">' . $gen_meta_keywords . '</div>';
			$html[] = '<div class="bbit-post-meta-title">' . $post_metas['title'] . '</div>';
			$html[] = '<div class="bbit-post-meta-description">' . $post_metas['description'] . '</div>';
			$html[] = '<div class="bbit-post-meta-keywords">' . $post_metas['keywords'] . '</div>';
			$html[] = '<div class="bbit-post-meta-focus-kw">' . $post_metas['focus_keyword'] . '</div>';
			$html[] = '<div class="bbit-post-meta-canonical">' . $post_metas['canonical'] . '</div>';
			$html[] = '<div class="bbit-post-meta-robots-index">' . $post_metas['robots_index'] . '</div>';
			$html[] = '<div class="bbit-post-meta-robots-follow">' . $post_metas['robots_follow'] . '</div>';
		...
		public function edit_post_inline_boxtpl() {
			/*
					<div>
						...
						<input type="text" class="large-text" style="width: 300px;" value="" name="bbit-editpost-meta-focus-kw" id="bbit-editpost-meta-focus-kw">
					</div>
			*/
			$html = '
	<table class="bbit-inline-edit-post form-table" style="border: 1px solid #dadada;">
		<thead>
			<tr>
			...
			<tr>
				<td width="45%">
					<div>
						...
						<input type="text" class="" style="" value="" name="bbit-editpost-meta-title" id="bbit-editpost-meta-title">
					</div>
					<div>
						<span>' . __('Canonical URL:', $this->localizationName) . '</span>
						<input type="text" class="" style="" value="" name="bbit-editpost-meta-canonical" id="bbit-editpost-meta-canonical">
					</div>
					<div>
						<span>' . __('Meta Robots Index:', $this->localizationName) . '</span>
						<select name="bbit-editpost-meta-robots-index" id="bbit-editpost-meta-robots-index">
					...
					<div>
						...
						<select name="bbit-editpost-meta-robots-follow" id="bbit-editpost-meta-robots-follow">
				...
				<td>
					<textarea name="bbit-editpost-meta-description" id="bbit-editpost-meta-description" rows="3" class="large-text"></textarea>
				</td>
				<td>
					<textarea name="bbit-editpost-meta-keywords" id="bbit-editpost-meta-keywords" rows="3" class="large-text"></textarea>
				</td>
			</tr>
			<tr>
				<td colspan=3>
					<div style="float:left; width:100%;">
						<input type="button" value="' . __('Cancel', $this->localizationName) . '" id="bbit-inline-btn-cancel" class="bbit-button gray" style="float:left;">
						<input type="button" value="' . __('Save', $this->localizationName) . '" id="bbit-inline-btn-save" class="bbit-button blue" style="float:right;">
					</div>
				</td>
			</tr>
		</tbody>
	    ...
	    public function __tax_get_post_meta( $post_meta=null, $post=null, $key='' ) {
			...
			$bbit_taxonomy_seo = $post_meta;
	    	if ( is_null($post_meta) ) {
	    		$bbit_taxonomy_seo = get_option( 'bbit_taxonomy_seo' );
	    		if ( $bbit_taxonomy_seo===false )
	    			return null;
	    	}
	    	if ( is_null($bbit_taxonomy_seo) )
	    		return null;
	    	if ( empty($bbit_taxonomy_seo) )
				return null;

			if ( is_null($post_meta) )
				$bbit_current_taxseo = $bbit_taxonomy_seo[ "{$post->taxonomy}" ][ "{$post->term_id}" ];
			else
				$bbit_current_taxseo = $post_meta;

    		if ( !isset($bbit_current_taxseo) || !is_array($bbit_current_taxseo) )
	    			return null;

	    	if ( $key=='' )
	    		return $bbit_current_taxseo;

	    	if ( isset($bbit_current_taxseo[ "$key" ]) )
	    		return $bbit_current_taxseo[ "$key" ];
	    	return null;
	    }
	    ...
	    public function __tax_update_post_meta( $post=null, $keyval=array() ) {
    		...
    		if ( !is_array($bbit_current_taxseo) )
    			$bbit_current_taxseo = array();

			foreach ( $keyval as $key => $value ) {
				if ( isset($bbit_current_taxseo[" $key "]) )
					unset( $bbit_current_taxseo[" $key "] );
				$bbit_current_taxseo[ "$key" ] = $value;
			}

			$bbit_taxonomy_seo[ "{$post->taxonomy}" ][ "{$post->term_id}" ] = $bbit_current_taxseo;				
			update_option( 'bbit_taxonomy_seo', $bbit_taxonomy_seo );
	    }
		...
		public function smushit_show_sizes_msg_details( $meta=array(), $show_sizes=true ) {
			...
			$ret = array();
			if ( !isset($meta['bbit_smushit']) || empty($meta['bbit_smushit']) ) return $ret;

			$ret[] = $meta['bbit_smushit']['msg'];
		...
		public function capabilities_user_has_module( $module='' ) {
			...
			$capabilitiesRoles = $this->get_theoption('bbit_capabilities_roles');
		...
		private function cronjobs_clean_fix() {
			$alreadyCleaned = get_option('bbit_cronjobs_clean');
			...
			// clean cronjobs
			if ( $doit ) {
				$this->cronjobs_clear_all_crons('bbitwplannerhourlyevent');
				$this->cronjobs_clear_all_crons('bbit_wplanner_hourly_event');
				$this->cronjobs_clear_all_crons('bbit_start_cron_serp_check');
				
				update_option( 'bbit_cronjobs_clean', 'done' );
			}
		}
		...
		private function fix_backlinkbuilder_linklist() {
			$alreadyCleaned = get_option('bbit_fix_backlinkbuilder');
			...
			if ( $doit ) {
				...
				update_option( 'bbit_fix_backlinkbuilder', 'done' );
			}
		}
		...
		public function admin_notice_details() {
			...
			if ( is_plugin_active( 'premium-seo-pack/plugin.php' ) ) {
				$__moduleIsActive = get_option('bbit_module_Social_Stats');
				$__submoduleSocialShare = get_option('bbit_socialsharing');
		...
		<div id="message" class="updated aaFrm-message_activate wc-connect">
			<div class="squeezer">
				<h4><?php _e( 'AA Social Share notice: you already use Bbit Plugin - Social Stats module, Social Sharing section', $this->localizationName ); ?></h4>
			</div>
		</div>
		<?php	
		}
		...
		public function print_module_error( $module=array(), $error_number, $title="" )
		{
			...
			$html[] = '<div class="bbit-grid_4 bbit-error-using-module">';
			$html[] = 	'<div class="bbit-panel">';
			$html[] = 		'<div class="bbit-panel-header">';
			$html[] = 			'<span class="bbit-panel-title">';
			$html[] = 				__( $title, $this->localizationName );
			$html[] = 			'</span>';
			$html[] = 		'</div>';
			$html[] = 		'<div class="bbit-panel-content">';
			
			$error_msg = isset($module[$module['alias']]['errors'][$error_number]) ? $module[$module['alias']]['errors'][$error_number] : '';
			
			$html[] = 			'<div class="bbit-error-details">' . ( $error_msg ) . '</div>';
			$html[] = 		'</div>';
			$html[] = 	'</div>';
			$html[] = '</div>';
