...
class bbitSeoCheck
{
    ...
    * Singleton pattern
    *
    * @return bbitSeoCheck Singleton instance
    */
	...
	public function get_seo_score( $p=0, $kw='', $returnAs='die', $post_content='empty' )
	{
		...
		if( count($post) > 0 ){
			...
			if ( $this->the_plugin->__tax_istax( $p ) ) { //taxonomy data!
				...
				$bbit_current_taxseo = $this->the_plugin->__tax_get_post_meta( null, $p );
				if ( is_null($bbit_current_taxseo) || !is_array($bbit_current_taxseo) )
					$bbit_current_taxseo = array();

				$post_metas = $this->the_plugin->__tax_get_post_meta( $bbit_current_taxseo, $p, 'bbit_meta' );

			} else
				$post_metas = get_post_meta( (int) $p, 'bbit_meta', true);
	...
	*/
	public function save_seo_score( $post_id=0, $status=array(), $score=0, $kw='' )
	{
		if( count($status) > 0 && $post_id > 0 ){
			...
			update_post_meta( $post_id, 'bbit_status', $status );
			update_post_meta( $post_id, 'bbit_score', $score );
			update_post_meta( $post_id, 'bbit_kw', $kw );
		}
	}
	...
	*/
	...
	function score_embedded_content( $page_content )
	{
		if ( !empty($this->the_plugin->charset) )
			$html = bbitphpQuery::newDocumentHTML( $page_content, $this->the_plugin->charset );
		else
			$html = bbitphpQuery::newDocumentHTML( $page_content );
