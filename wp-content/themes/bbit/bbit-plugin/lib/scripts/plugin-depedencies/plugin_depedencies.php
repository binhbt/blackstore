    ...
    class aaTeamPluginDepedencies
    {
	    ...
	    */
		...
		public function depedencies_plugin_redirect_valid() {
			//delete_option('bbit_depedencies_is_valid');
		...
		public function depedencies_plugin_redirect() {
			...
			delete_option('bbit_depedencies_do_activation_redirect');
			wp_redirect( get_admin_url() . 'admin.php?page=bbit' );
		}
		...
		public function verifyDepedencies() {
			...
			?>
			<div class="bbit-message">
				 All of the bellow libraries must be enabled in order for our plugin to function right!
			</div>
			<input type="button" value="Re-Check Depedencies" class="bbit-button blue bbit-depedencies-check" style="margin-top: 10px;">
			<?php
